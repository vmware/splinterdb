...
def get_ycsb_string(out_dir, datetime_str, threads, dram_size, pmem_size, value_size):
    """generate output filename for result file"""
    return "{}/results_value_{}-thr_{}-mem_[{},{}]-{}.csv".format(
        out_dir, value_size, threads, bytes_to_str(dram_size), bytes_to_str(pmem_size), datetime_str)

def get_workload_string(filename, threads, dram_size, pmem_size, value_size):
    """generate output filename for workload (trace)"""
    return "{}-value_{}-thr_{}-mem_[{},{}]".format(
            filename, value_size, threads, bytes_to_str(dram_size), bytes_to_str(pmem_size))

# main

# parse commmand line arguments
DESC = "SplinterDB Trace Replayer. Replays all traces in trace_list and outputs statistics to \
        output_dir. If --memory and --pmem, and/or --threads are set, then runs all traces for \
        each combination of thread_count and/or memory_size pairs."
...
CL_PARSER.add_argument('--dram', nargs="+", type=int, default=[2],
                       help="List of DRAM sizes to use in GiB, each will be run")
CL_PARSER.add_argument('--pmem', nargs="+", type=int, default=[2],
                       help="List of PMEM sizes to use in GiB, each will be run")
