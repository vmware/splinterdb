...
static inline bool memtable_try_transition(memtable *mt,
  ...
  if (actual_old_state != old_state) {
    switch (old_state) {
    case MEMTABLE_STATE_COMPACTED:
      debug_assert(actual_old_state != MEMTABLE_STATE_INCORPORATION_ASSIGNED);
...
static inline void memtable_transition(memtable *mt, memtable_state old_state,
                                       memtable_state new_state) {
...
platform_status memtable_maybe_rotate_and_get_insert_lock(
    memtable_context *ctxt, uint64 *generation, page_handle **lock_page);

void memtable_unget_insert_lock(memtable_context *ctxt, page_handle *lock_page);

platform_status memtable_insert(memtable_context *ctxt, memtable *mt,
                                const char *key, const char *data,
                                uint64 *generation);

page_handle *memtable_get_lookup_lock(memtable_context *ctxt);

void memtable_unget_lookup_lock(memtable_context *ctxt, page_handle *lock_page);

page_handle *
memtable_uncontended_get_claim_lock_lookup_lock(memtable_context *ctxt);

void memtable_unlock_unclaim_unget_lookup_lock(memtable_context *ctxt,
                                               page_handle *lock_page);

bool memtable_dec_ref_maybe_recycle(memtable_context *ctxt, memtable *mt);

uint64 memtable_force_finalize(memtable_context *ctxt);

void memtable_init(memtable *mt, cache *cc, memtable_config *cfg,
                   uint64 generation);

void memtable_deinit(cache *cc, memtable *mt);

memtable_context *memtable_context_create(platform_heap_id hid, cache *cc,
                                          memtable_config *cfg,
                                          process_fn process,
                                          void *process_ctxt);

void memtable_context_destroy(platform_heap_id hid, memtable_context *ctxt);

void memtable_config_init(memtable_config *cfg, btree_config *btree_cfg,
                          uint64 max_memtables, uint64 memtable_capacity);

static inline uint64 memtable_root_addr(memtable *mt) { return mt->root_addr; }

static inline uint64 memtable_generation(memtable_context *ctxt) {
...
static inline uint64 memtable_generation_retired(memtable_context *ctxt) {
...
static inline void memtable_lock_incorporation_lock(memtable_context *ctxt) {
  platform_mutex_lock(&ctxt->incorporation_mutex);
}

static inline void memtable_unlock_incorporation_lock(memtable_context *ctxt) {
  platform_mutex_unlock(&ctxt->incorporation_mutex);
}

static inline void memtable_zap(cache *cc, memtable *mt) {
  btree_zap(cc, mt->cfg, mt->root_addr, PAGE_TYPE_MEMTABLE);
}

static inline bool memtable_ok_to_lookup(memtable *mt) {
...
static inline bool memtable_ok_to_lookup_compacted(memtable *mt) {
  // FIXME: [aconway 2020-09-02]: save state and check
  return mt->state == MEMTABLE_STATE_COMPACTED ||
         mt->state == MEMTABLE_STATE_INCORPORATION_ASSIGNED;
}

bool memtable_is_empty(memtable_context *mt_ctxt);

static inline bool memtable_verify(cache *cc, memtable *mt) {
  return btree_verify_tree(cc, mt->cfg, mt->root_addr, PAGE_TYPE_MEMTABLE);
}

static inline void memtable_print(cache *cc, memtable *mt) {
  btree_print_tree(cc, mt->cfg, mt->root_addr);
}

static inline void memtable_print_stats(cache *cc, memtable *mt) {
  btree_print_tree_stats(cc, mt->cfg, mt->root_addr);
};

static inline void memtable_key_to_string(memtable *mt, const char *key,
                                          char *key_str) {
