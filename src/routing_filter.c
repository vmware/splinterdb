...
static uint32 *RadixSort(uint32 *pData,
                         uint32 mBuf[static MATRIX_ROWS * MATRIX_COLS],
                         uint32 *pTemp, uint32 count, uint32 fp_size,
                         uint32 value_size) {
...
__attribute__((unused)) static inline void routing_set_bit(uint64 *data,
                                                           uint64 bitnum) {
  *(data + bitnum / 64) |= (1ULL << (bitnum % 64));
}

static inline void routing_unset_bit(uint64 *data, uint64 bitnum) {
  *(data + bitnum / 64) &= ~((1ULL << (bitnum % 64)));
}

static inline uint32 routing_get_bucket(uint32 fp,
                                        size_t remainder_and_value_size) {
  return fp >> remainder_and_value_size;
}

static inline uint32 routing_get_index(uint32 fp,
...
static inline void
routing_filter_get_remainder_and_value(routing_config *cfg, uint32 *data,
                                       uint32 pos, uint32 *remainder_and_value,
                                       size_t remainder_value_size) {
  *remainder_and_value = PackedArray_get(data, pos, remainder_value_size);
}

static inline routing_hdr *routing_get_header(cache *cc, routing_config *cfg,
...
static inline void routing_unget_header(cache *cc, page_handle *header_page) {
  cache_unget(cc, header_page);
}

static inline uint64 routing_header_length(routing_config *cfg,
                                           routing_hdr *hdr) {
  uint64 metamessage_size = (hdr->num_remainders + cfg->index_size - 1) / 8 + 4;
  return metamessage_size + sizeof(routing_hdr);
}

static inline page_handle *routing_get_and_lock_page(cache *cc, uint64 addr) {
...
static inline void routing_unlock_and_unget_page(cache *cc, page_handle *page) {
...
static inline void routing_get_bucket_bounds(char *encoding, uint64 len,
...
void routing_get_bucket_counts(routing_config *cfg, routing_hdr *hdr,
                               uint32 *count) {
  uint64 start = 0;
  uint64 end;
  uint64 i;
...
platform_status routing_filter_add(cache *cc, routing_config *cfg,
                                   platform_heap_id hid,
                                   routing_filter *old_filter,
                                   routing_filter *filter, uint32 *new_fp_arr,
                                   uint64 num_new_fp, uint16 value) {
  ...
  fp_no = 0;
  for (uint32 old_index_no = 0; old_index_no < old_num_indices;
       old_index_no++) {
    ...
    if (old_filter->addr != 0) {
      routing_hdr *old_hdr = routing_get_header(cc, cfg, old_filter->addr,
                                                old_index_no, &old_filter_node);
    ...
    if (old_index_count != 0) {
      PackedArray_unpack((uint32 *)old_block_start, 0, old_src_fp,
                         old_index_count, old_remainder_and_value_size);
      uint32 old_fp_no = 0;
...
void routing_filter_prefetch(cache *cc, routing_config *cfg,
                             routing_filter *filter, uint64 num_indices) {
...
uint32 routing_filter_estimate_unique_fp(cache *cc, routing_config *cfg,
                                         platform_heap_id hid,
...
platform_status routing_filter_lookup(cache *cc, routing_config *cfg,
                                      routing_filter *filter, const char *key,
                                      uint64 *found_values) {
  if (filter->addr == 0) {
    *found_values = 0;
    return STATUS_OK;
  ...
  for (uint32 i = 0; i < end - start; i++) {
    ...
    routing_filter_get_remainder_and_value(cfg, (uint32 *)remainder_block_start,
                                           pos, &found_remainder_and_value,
                                           remainder_and_value_size);
...
static inline void routing_async_set_state(routing_async_ctxt *ctxt,
...
static void routing_filter_async_callback(cache_async_ctxt *cache_ctxt) {
...
cache_async_result routing_filter_lookup_async(cache *cc, routing_config *cfg,
                                               routing_filter *filter,
                                               char *key, uint64 *found_values,
  ...
  do {
    ...
    case routing_async_state_start: {
      ...
      uint64 addrs_per_page = cfg->page_size / sizeof(uint64);
      ctxt->page_addr =
          filter->addr + cfg->page_size * (ctxt->index / addrs_per_page);
    ...
    case routing_async_state_get_filter: {
      ...
      res = cache_get_async(cc, ctxt->page_addr, PAGE_TYPE_FILTER, cache_ctxt);
      switch (res) {
      case async_locked:
      case async_no_reqs:
    ...
    case routing_async_state_got_filter: {
      ...
      routing_hdr *hdr = (routing_hdr *)(cache_ctxt->page->data +
                                         (ctxt->header_addr % cfg->page_size));
      ...
      for (uint32 i = 0; i < end - start; i++) {
        ...
        routing_filter_get_remainder_and_value(
            cfg, (uint32 *)remainder_block_start, pos,
...
void routing_filter_zap(cache *cc, routing_filter *filter) {
...
uint32 routing_filter_estimate_unique_keys_from_count(routing_config *cfg,
                                                      uint64 num_unique) {
...
uint32 routing_filter_estimate_unique_keys(routing_filter *filter,
                                           routing_config *cfg) {
...
void routing_filter_verify(cache *cc, routing_config *cfg,
                           routing_filter *filter, uint16 value,
                           iterator *itor) {
  bool at_end;
...
void routing_filter_print_encoding(routing_config *cfg, routing_hdr *hdr) {
  uint32 i;
...
void routing_filter_print_index(cache *cc, routing_config *cfg,
                                uint64 filter_addr, uint32 num_indices) {
  uint64 i;
...
void routing_filter_print_remainders(routing_config *cfg, routing_hdr *hdr,
                                     size_t remainder_size, size_t value_size) {
  ...
  for (i = 0; i < cfg->index_size; i++) {
    ...
    for (j = start; j < end; j++) {
      ...
      routing_filter_get_remainder_and_value(
          cfg, (uint32 *)((char *)hdr + header_length), j, &remainder_and_value,
          remainder_and_value_size);
...
void routing_filter_print(cache *cc, routing_config *cfg,
                          routing_filter *filter) {
