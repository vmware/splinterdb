...
void task_init_tid_bitmask(uint64 *tid_bitmask) {
...
static void init_threadid(task_system *ts) {
...
threadid task_get_max_tid(task_system *ts) {
  threadid *max_tid = task_system_get_max_tid(ts);
  /*
...
void task_init_thread_task(task_system *ts, const threadid tid,
                           thread_task *task, void *task_scratch) {
  thread_task **thread_lookup = task_system_get_system_threads(ts);
  thread_lookup[tid] = task;
  task->scratch = task_scratch;
}

bool task_is_threadid_allocated(task_system *ts, threadid tid) {
  uint64 *tid_bitmask = task_system_get_tid_bitmask(ts);
  // check for bit to be set to 0.
  return !(*tid_bitmask & (1ULL << tid));
}

void task_clear_threadid(task_system *ts, threadid tid) {
  uint64 *tid_bitmask = task_system_get_tid_bitmask(ts);
...
platform_status task_register_hook(task_hook newhook) {
...
static void task_start_thread_with_hooks(void *func_and_args) {
...
platform_status task_create_thread_with_hooks(
    platform_thread *thread, bool detached, platform_thread_worker func,
    void *arg, size_t scratch_size, task_system *ts, platform_heap_id hid) {
  platform_status ret;
  thread_task *thread_to_create = TYPED_ZALLOC_MANUAL(
...
platform_status task_thread_create(const char *name,
                                   platform_thread_worker func, void *arg,
                                   size_t scratch_size, task_system *ts,
                                   platform_heap_id hid,
                                   platform_thread *thread) {
  platform_thread thr;
  platform_status ret;

  ret = task_create_thread_with_hooks(&thr, FALSE, func, arg, scratch_size, ts,
                                      hid);
  if (!SUCCESS(ret)) {
...
void task_worker_thread(void *arg) {
...
static void task_group_stop_and_wait_for_threads(task_group *group) {
...
void task_group_deinit(task_group *group) {
...
static platform_status task_group_init(task_group *group, task_system *ts,
                                       bool use_stats, bool use_bg_threads,
  ...
  if (use_bg_threads) {
    ...
    for (uint8 i = 0; i < num_bg_threads; i++) {
      rc = task_thread_create("splinter-bg-thread", task_worker_thread,
                              (void *)group, scratch_size, ts, hid,
                              &group->bg.threads[i]);
      if (!SUCCESS(rc)) {
        task_group_stop_and_wait_for_threads(group);
        goto out;
...
static inline platform_status task_lock_task_queue(task_group *group) {
  task_system *ts = group->ts;
  return task_system_use_bg_threads(ts) ? platform_condvar_lock(&group->bg.cv)
                                        : platform_mutex_lock(&group->fg.mutex);
}

static inline platform_status task_unlock_task_queue(task_group *group) {
  task_system *ts = group->ts;
...
platform_status task_enqueue(task_system *ts, task_type type, task_fn func,
                             void *arg, bool at_head) {
...
void task_perform_all(task_system *ts) {
...
static inline platform_status task_group_perform_one(task_group *group) {
  platform_assert(!task_system_use_bg_threads(group->ts));
  platform_status rc;
 ...
 */
platform_status task_perform_one(task_system *ts) {
...
platform_status
task_system_create(platform_heap_id hid, platform_io_handle *ioh,
                   task_system **system, bool use_stats, bool use_bg_threads,
                   uint8 num_bg_threads[NUM_TASK_TYPES], uint64 scratch_size) {
  task_system *ts =
  ...
  for (task_type type = 0; type != NUM_TASK_TYPES; type++) {
    platform_status rc =
        task_group_init(&ts->group[type], ts, use_stats, use_bg_threads,
                        num_bg_threads[type], scratch_size);
    if (!SUCCESS(rc)) {
...
bool task_system_use_bg_threads(task_system *ts) { return ts->use_bg_threads; }

thread_task **task_system_get_system_threads(task_system *ts) {
  return ts->thread_tasks;
}

void task_system_io_register_thread(task_system *ts) {
  io_thread_register(&ts->ioh->super);
}

void task_system_register_thread(task_system *ts) {
  task_run_thread_hooks(ts);
  const threadid tid = platform_get_tid();
  // Only register init thread
  platform_assert(ts->init_tid == INVALID_TID);
  task_init_thread_task(ts, tid, &ts->init_thread_task, ts->init_task_scratch);
}

void task_system_destroy(platform_heap_id hid, task_system *ts) {
  for (task_type type = 0; type != NUM_TASK_TYPES; type++) {
...
uint64 *task_system_get_tid_bitmask(task_system *ts) {
  return &ts->tid_bitmask;
}

threadid *task_system_get_max_tid(task_system *ts) { return &ts->max_tid; }

void *task_system_get_thread_scratch(task_system *ts, const threadid tid) {
  thread_task *thread = task_system_get_system_threads(ts)[tid];
  return thread->scratch;
}

void task_wait_for_completion(task_system *ts) {
  for (task_type type = 0; type != NUM_TASK_TYPES; type++) {
    task_group *group = &ts->group[type];
...
static void task_group_print_stats(task_group *group, task_type type) {
  if (!group->use_stats) {
...
void task_print_stats(task_system *ts) {
  for (task_type type = 0; type != NUM_TASK_TYPES; type++) {
