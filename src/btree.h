...
static inline void btree_ctxt_init(btree_async_ctxt *ctxt,       // OUT
...
void btree_lookup_with_ref(cache *cc, btree_config *cfg, uint64 root_addr,
                           btree_node *node, page_type type, const char *key,
                           char **data, bool *found);

cache_async_result btree_lookup_async_with_ref(cache *cc, btree_config *cfg,
                                               uint64 root_addr,
                                               btree_node *node, char *key,
                                               char **data, bool *found,
                                               btree_async_ctxt *ctxt);

void btree_node_unget(cache *cc, btree_config *cfg, btree_node *node);
void btree_lookup(cache *cc, btree_config *cfg, uint64 root_addr,
                  const char *key, char *data, bool *found);

cache_async_result btree_lookup_async(cache *cc, btree_config *cfg,
                                      uint64 root_addr, char *key, char *data,
                                      bool *found, btree_async_ctxt *ctxt);
...
void btree_iterator_init(cache *cc, btree_config *cfg, btree_iterator *iterator,
...
static inline void btree_pack_req_init(btree_pack_req *req, cache *cc,
                                       btree_config *cfg, iterator *itor,
                                       uint64 max_tuples, hash_fn hash,
                                       unsigned int seed,
...
static inline void btree_pack_req_deinit(btree_pack_req *req,
                                         platform_heap_id hid) {
...
uint64 btree_rough_count(cache *cc, btree_config *cfg, uint64 root_addr,
                         char *min_key, char *max_key);

void btree_print_tree(cache *cc, btree_config *cfg, uint64 addr);
...
void btree_print_lookup(cache *cc, btree_config *cfg, uint64 root_addr,
                        page_type type, char *key);
...
static inline uint64 btree_key_size(btree_config *cfg) {
  return cfg->data_cfg->key_size;
}

static inline uint64 btree_message_size(btree_config *cfg) {
  return cfg->data_cfg->message_size;
}

static inline uint64 btree_tuple_size(btree_config *cfg) {
  return btree_key_size(cfg) + btree_message_size(cfg);
}

static inline char *btree_min_key(btree_config *cfg) {
  return cfg->data_cfg->min_key;
}

static inline int btree_key_compare(btree_config *cfg, const char *key1,
                                    const char *key2) {
  return data_key_compare(cfg->data_cfg, key1, key2);
}

static inline void btree_key_to_string(btree_config *cfg, const char *key,
                                       char str[static 128]) {
  return data_key_to_string(cfg->data_cfg, key, str, 128);
}

static inline void btree_message_to_string(btree_config *cfg, const char *data,
                                           char str[static 128]) {
