...
static inline uint32 platform_popcount(uint32 x) {
...
static inline bool platform_checksum_is_equal(checksum128 left,
                                              checksum128 right) {
...
static inline timestamp platform_timestamp_diff(timestamp start,
                                                timestamp end) {
  return end - start;
}
...
static inline void platform_pause() { __builtin_ia32_pause(); }
...
static inline void
platform_semaphore_init(platform_semaphore *sema, int value,
...
// platform predicates
static inline bool STATUS_IS_EQ(const platform_status s1,
                                const platform_status s2) {
  return s1.r == s2.r;
}

static inline bool STATUS_IS_NE(const platform_status s1,
                                const platform_status s2) {
  return s1.r != s2.r;
}
...
static inline void platform_histo_insert(platform_histo_handle histo,
                                         int64 datum) {
...
static inline void platform_histo_merge_in(platform_histo_handle dest_histo,
...
static inline platform_heap_id platform_get_heap_id(void) {
  // void* NULL since we don't actually need a heap id
  return NULL;
}

static inline platform_module_id platform_get_module_id() {
...
static inline platform_status platform_condvar_lock(platform_condvar *cv) {
  int status;
...
static inline platform_status platform_condvar_unlock(platform_condvar *cv) {
  int status;
...
static inline void platform_condvar_destroy(platform_condvar *cv) {
