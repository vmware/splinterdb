...
platform_status laio_write(io_handle *ioh, void *buf, uint64 bytes,
                           uint64 addr);
...
platform_status laio_read_async(io_handle *ioh, io_async_req *req,
                                io_callback_fn callback, uint64 count,
                                uint64 addr);
platform_status laio_write_async(io_handle *ioh, io_async_req *req,
                                 io_callback_fn callback, uint64 count,
                                 uint64 addr);
...
platform_status io_handle_init(laio_handle *io, io_config *cfg,
...
void io_handle_deinit(laio_handle *io) {
  int status;
...
platform_status laio_read(io_handle *ioh, void *buf, uint64 bytes,
                          uint64 addr) {
  laio_handle *io;
  int ret;
...
void laio_callback(io_context_t ctx, struct iocb *iocb, long res, long res2) {
  io_async_req *req;
