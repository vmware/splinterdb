...
platform_status platform_heap_create(platform_module_id UNUSED_PARAM(module_id),
                                     uint32 max,
...
void platform_heap_destroy(platform_heap_handle UNUSED_PARAM(*heap_handle)) {}
...
void *platform_buffer_getaddr(const buffer_handle *bh) { return bh->addr; }
...
platform_status platform_histo_create(platform_heap_id heap_id,
  ...
  hh = TYPED_MALLOC_MANUAL(heap_id, hh,
                           sizeof(hh) + num_buckets * sizeof(hh->count[0]));
  if (!hh) {
...
char *platform_strtok_r(char *str, const char *delim,
                        platform_strtok_ctx *ctx) {
