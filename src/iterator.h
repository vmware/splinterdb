...
// FIXME: [nsarmicanic 2020-07-16] Might be better to return type instead of
// void
...
static inline void iterator_get_curr(iterator *itor, char **key, char **data,
                                     data_type *type) {
  itor->ops->get_curr(itor, key, data, type);
}

static inline platform_status iterator_at_end(iterator *itor, bool *at_end) {
  return itor->ops->at_end(itor, at_end);
}

static inline platform_status iterator_advance(iterator *itor) {
  return itor->ops->advance(itor);
}

static inline void iterator_print(iterator *itor) {
