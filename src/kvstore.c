                  ...
                  KVStore *kvs)                 // OUT
{
   ...
   splinter_config_init(&kvs->splinterCfg, &kvs->dataCfg, NULL,
                        ...
                        masterCfg.page_size, masterCfg.extent_size,
             ...
             KVStoreHandle *kvsHandle)          // OUT
{
   ...
   status = io_handle_init(&kvs->ioHandle, &kvs->ioCfg, kvs->heapHandle,
                           kvs->heapID);
   if (!SUCCESS(status)) {
      platform_error_log("Failed to init io handle: %s\n",
                         platform_status_to_string(status));
   ...
   status = clockcache_init(&kvs->cacheHandle, &kvs->cacheCfg,
                            (io_handle *)&kvs->ioHandle,
                            (allocator *)&kvs->allocatorHandle, "kvStore",
                            kvs->taskSystem, kvs->heapHandle, kvs->heapID,
