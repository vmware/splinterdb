...
typedef platform_status (*extent_alloc_fn)(
    cache *cc, page_handle *page_arr[static MAX_PAGES_PER_EXTENT],
    page_type type);
...
typedef page_handle *(*page_get_fn)(cache *cc, uint64 addr, bool blocking,
                                    page_type type);
typedef cache_async_result (*page_get_async_fn)(cache *cc, uint64 addr,
                                                page_type type,
                                                cache_async_ctxt *ctxt);
typedef void (*page_async_done_fn)(cache *cc, page_type type,
                                   cache_async_ctxt *ctxt);
...
typedef void (*page_sync_fn)(cache *cc, page_handle *page, bool is_blocking,
                             page_type type);
typedef uint64 (*extent_sync_fn)(cache *cc, uint64 addr,
                                 uint64 *pages_outstanding);

typedef void (*share_fn)(cache *cc, page_handle *page_to_share,
...
static inline bool cache_dealloc(cache *cc, uint64 addr, page_type type) {
  return cc->ops->page_dealloc(cc, addr, type);
}

static inline uint8 cache_get_ref(cache *cc, uint64 addr) {
  return cc->ops->page_get_ref(cc, addr);
}

static inline page_handle *cache_get(cache *cc, uint64 addr, bool blocking,
                                     page_type type) {
  return cc->ops->page_get(cc, addr, blocking, type);
}

static inline void cache_ctxt_init(cache *cc, cache_async_cb cb, void *cbdata,
                                   cache_async_ctxt *ctxt) {
  ctxt->cc = cc;
  ctxt->cb = cb;
  ctxt->cbdata = cbdata;
...
static inline cache_async_result cache_get_async(cache *cc, uint64 addr,
                                                 page_type type,
                                                 cache_async_ctxt *ctxt) {
  return cc->ops->page_get_async(cc, addr, type, ctxt);
}

static inline void cache_async_done(cache *cc, page_type type,
                                    cache_async_ctxt *ctxt) {
  return cc->ops->page_async_done(cc, type, ctxt);
}

static inline void cache_unget(cache *cc, page_handle *page) {
  return cc->ops->page_unget(cc, page);
}

static inline bool cache_claim(cache *cc, page_handle *page) {
  return cc->ops->page_claim(cc, page);
}

static inline void cache_unclaim(cache *cc, page_handle *page) {
  return cc->ops->page_unclaim(cc, page);
}

static inline void cache_lock(cache *cc, page_handle *page) {
  return cc->ops->page_lock(cc, page);
}

static inline void cache_unlock(cache *cc, page_handle *page) {
  return cc->ops->page_unlock(cc, page);
}

static inline bool cache_prefetch(cache *cc, uint64 addr, bool full_extent) {
  return cc->ops->page_prefetch(cc, addr, full_extent);
}

static inline void cache_share(cache *cc, page_handle *page_to_share,
                               page_handle *anon_page) {
  return cc->ops->share(cc, page_to_share, anon_page);
}

static inline void cache_unshare(cache *cc, page_handle *anon_page) {
  return cc->ops->unshare(cc, anon_page);
}

static inline void cache_mark_dirty(cache *cc, page_handle *page) {
  return cc->ops->page_mark_dirty(cc, page);
}

static inline void cache_pin(cache *cc, page_handle *page) {
  return cc->ops->page_pin(cc, page);
}

static inline void cache_unpin(cache *cc, page_handle *page) {
  return cc->ops->page_unpin(cc, page);
}

static inline void cache_page_sync(cache *cc, page_handle *page,
                                   bool is_blocking, page_type type) {
  return cc->ops->page_sync(cc, page, is_blocking, type);
}

static inline void cache_extent_sync(cache *cc, uint64 addr,
                                     uint64 *pages_outstanding) {
  cc->ops->extent_sync(cc, addr, pages_outstanding);
}

static inline void cache_flush(cache *cc) { cc->ops->flush(cc); }

static inline int cache_evict(cache *cc, bool ignore_pinned_pages) {
  return cc->ops->evict(cc, ignore_pinned_pages);
}

static inline void cache_cleanup(cache *cc) { return cc->ops->cleanup(cc); }

static inline uint64 cache_page_size(cache *cc) {
  return cc->ops->get_page_size(cc);
}

static inline uint64 cache_extent_size(cache *cc) {
  return cc->ops->get_extent_size(cc);
}

static inline void cache_assert_ungot(cache *cc, uint64 addr) {
  return cc->ops->assert_ungot(cc, addr);
}

static inline void cache_assert_free(cache *cc) {
  return cc->ops->assert_free(cc);
}

static inline void cache_assert_noleaks(cache *cc) {
  return cc->ops->assert_noleaks(cc);
}

static inline void cache_print(cache *cc) { return cc->ops->print(cc); }

static inline void cache_print_stats(cache *cc) {
  return cc->ops->print_stats(cc);
}

static inline void cache_reset_stats(cache *cc) {
  return cc->ops->reset_stats(cc);
}

static inline void cache_io_stats(cache *cc, uint64 *read_bytes,
                                  uint64 *write_bytes) {
  return cc->ops->io_stats(cc, read_bytes, write_bytes);
}

static inline bool cache_page_valid(cache *cc, uint64 addr) {
  return cc->ops->page_valid(cc, addr);
}

static inline void cache_validate_page(cache *cc, page_handle *page,
                                       uint64 addr) {
  cc->ops->validate_page(cc, page, addr);
}

static inline uint32 cache_count_dirty(cache *cc) {
  return cc->ops->count_dirty(cc);
}

static inline uint32 cache_get_read_ref(cache *cc, page_handle *page) {
  return cc->ops->page_get_read_ref(cc, page);
}

static inline bool cache_present(cache *cc, page_handle *page) {
  return cc->ops->cache_present(cc, page);
}

static inline void cache_enable_sync_get(cache *cc, bool enabled) {
  cc->ops->enable_sync_get(cc, enabled);
}

static inline allocator *cache_allocator(cache *cc) {
