...
void merge_get_curr(iterator *itor, char **key, char **data, data_type *type);
...
static void range_stack_init(range_stack *stack, int num_trees) {
...
static inline bool range_stack_makes_point_redundant(range_stack *stack,
                                                     message_type class) {
  return stack->size > 0 && class == MESSAGE_TYPE_DELETE;
}

static inline bool range_stack_clobbers_point(range_stack *stack,
                                              int point_seq) {
...
static inline char *range_stack_get_start_key(range_stack *stack) {
  return stack->start_key;
}

// end_key is the data of range delete (key, data)
static inline char *range_stack_get_end_key(range_stack *stack) {
  debug_assert(stack->end_seq >= 0);
  debug_assert(stack->end_seq < stack->num_sequences);
  return stack->limits[stack->end_seq];
}

static int find_predecessor(range_stack *stack, int seq) {
  /*
 ...
 *-----------------------------------------------------------------------------
 */
static int find_useful_successor(range_stack *stack, const char *limit, int seq,
                                 int insert_position, data_config *cfg) {
  /*
...
debug_range_stack_check_invariants(debug_only range_stack *stack,
  ...
  if (stack->size > 0) {
    ...
    int seq = stack->seq[0];
    debug_assert(data_key_compare(cfg, stack->start_key, stack->limits[seq]) <
                 0);
  } else {
...
static void add_new_range_delete_to_stack(range_stack *stack,
                                          const char *startkey,
                                          const char *limit, int seq,
                                          data_config *cfg) {
  ...
  if (predecessor >= 0 &&
      data_key_compare(cfg, stack->limits[stack->seq[predecessor]], limit) >=
          0) {
  ...
  int successor =
      find_useful_successor(stack, limit, seq, insert_position, cfg);
 ...
 *-----------------------------------------------------------------------------
 */
static bool maybe_remove_range_deletes(range_stack *stack, char *next_key,
                                       data_config *cfg) {
  bool r;
  debug_range_stack_check_invariants(stack, cfg);
  if (stack->size == 0) {
  ...
  int num_deleting = find_useful_successor(
      stack, next_key,
...
static inline int bsearch_comp(const ordered_iterator *itor_one,
                               const ordered_iterator *itor_two,
                               const data_config *cfg, bool *keys_equal) {
...
static int merge_comp(const void *one, const void *two, void *ctxt) {
...
static inline int bsearch_insert(register const ordered_iterator *key,
                                 ordered_iterator **base0, const size_t nmemb,
                                 const data_config *cfg, bool *prev_equal_out,
                                 bool *next_equal_out) {
...
static inline void set_curr_ordered_iterator(ordered_iterator *itor) {
  iterator_get_curr(itor->itor, &itor->key, &itor->data, &itor->type);
}

static inline void assert_data_config_valid(data_config *point_cfg,
...
debug_assert_message_type_valid(debug_only merge_iterator *merge_itor) {
  ...
  debug_assert(!merge_itor->has_data || !merge_itor->discard_deletes ||
               data == NULL || type != MESSAGE_TYPE_DELETE);
  debug_assert(!merge_itor->has_data || !merge_itor->resolve_updates ||
               data == NULL || type != MESSAGE_TYPE_UPDATE);
#endif
}

static void debug_verify_sorted(debug_only merge_iterator *merge_itor,
                                debug_only const int index) {
#if SPLINTER_DEBUG
  if (index < 0 || index + 1 >= merge_itor->num_remaining) {
    return;
  }
  const int cmp = data_key_compare(
      merge_itor->cfg, merge_itor->ordered_iterators[index]->key,
...
advance_and_resort_min_ritor(merge_iterator *merge_itor) {
  ...
  if (UNLIKELY(at_end)) {
    ...
    for (int i = 0; i < merge_itor->num_remaining; ++i) {
      merge_itor->ordered_iterators[i] = merge_itor->ordered_iterators[i + 1];
    }
  ...
  int index = 1 + bsearch_insert(*merge_itor->ordered_iterators,
                                 ...
                                 merge_itor->num_remaining - 1, merge_itor->cfg,
...
process_one_range_delete_or_clobber_one_point(merge_iterator *merge_itor) {
  ...
  case data_type_range:
    add_new_range_delete_to_stack(&merge_itor->stack, ordered_itor->key,
                                  ordered_itor->data, ordered_itor->seq,
                                  range_cfg);
    break;
  default:
    debug_assert(FALSE);
...
static platform_status merge_resolve_equal_keys(merge_iterator *merge_itor,
                                                bool *retry) {
  debug_assert(merge_itor->ordered_iterators[0]->next_key_equal);
  ...
  if (1 && merge_itor->ordered_iterators[0]->type == data_type_point &&
    ...
    do {
      if (1 && merge_itor->ordered_iterators[1]->type == data_type_point &&
          !range_stack_clobbers_point(&merge_itor->stack,
        ...
        // Verify keys match
        debug_assert(!data_key_compare(cfg, merge_itor->key,
 ...
 *-----------------------------------------------------------------------------
 */
static inline bool discard_range_deletes(merge_iterator *merge_itor) {
...
static platform_status advance_one_loop(merge_iterator *merge_itor,
                                        bool *retry) {
  *retry = FALSE;
    ...
    if (*retry) {
      return STATUS_OK;
             ...
             range_stack_clobbers_point(
                 &merge_itor->stack, merge_itor->ordered_iterators[0]->seq)) {
    /*
...
// FIXME: [yfogel 2020-08-24] Verify all input iterators are homogeneous for
// data_type
platform_status merge_iterator_create(platform_heap_id hid, data_config *cfg,
                                      data_config *range_cfg, int num_trees,
                                      iterator **itor_arr, bool discard_deletes,
                                      bool resolve_updates, bool has_data,
  ...
  // FIXME: [nsarmicanic 2020-07-31] remove if guard once range_cfg is never
  // passed as NULL
  if (range_cfg) {
  ...
  //      are fixed
  if (0 || !out_itor || !itor_arr || !cfg || num_trees < 0 ||
  ...
  for (i = -1; i < num_trees; i++) {
    ...
    merge_itor->ordered_iterators[i] = &merge_itor->ordered_iterator_stored[i];
  }
  ...
  for (i = 0; i + 1 < merge_itor->num_remaining; ++i) {
    int cmp =
        data_key_compare(merge_itor->cfg, merge_itor->ordered_iterators[i]->key,
...
platform_status merge_iterator_destroy(platform_heap_id hid,
                                       merge_iterator **merge_itor) {
...
platform_status merge_at_end(iterator *itor, // IN
                             bool *at_end)   // OUT
{
...
void merge_iterator_print(merge_iterator *merge_itor) {
  uint64 i;
