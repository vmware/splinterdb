...
void shard_log_iterator_get_curr(iterator *itor, char **key, char **data,
                                 data_type *type);
...
static inline checksum128 shard_log_checksum(shard_log_config *cfg,
                                             page_handle *page) {
...
uint64 shard_log_alloc(shard_log *log, uint64 *next_extent) {
  return mini_allocator_alloc(&log->mini, 0, NULL, next_extent);
}

platform_status shard_log_init(shard_log *log, cache *cc,
  ...
  for (threadid thr_i = 0; thr_i < MAX_THREADS; thr_i++) {
    shard_log_thread_data *thread_data = shard_log_get_thread_data(log, thr_i);
...
void shard_log_zap(shard_log *log) {
  cache *cc = log->cc;
...
int shard_log_write(log_handle *logh, char *key, char *data,
                    uint64 generation) {
...
uint64 shard_log_addr(log_handle *logh) {
  shard_log *log = (shard_log *)logh;
  return log->addr;
}

uint64 shard_log_meta_addr(log_handle *logh) {
  shard_log *log = (shard_log *)logh;
  return log->meta_head;
}

uint64 shard_log_magic(log_handle *logh) {
  shard_log *log = (shard_log *)logh;
  return log->magic;
}

bool shard_log_valid(shard_log_config *cfg, page_handle *page, uint64 magic) {
  shard_log_hdr *hdr = (shard_log_hdr *)page->data;
...
uint64 shard_log_next_extent_addr(shard_log_config *cfg, page_handle *page) {
  shard_log_hdr *hdr = (shard_log_hdr *)page->data;
  return hdr->next_extent_addr;
}

int shard_log_compare(const void *p1, const void *p2, void *unused) {
  return *(uint64 *)p1 - *(uint64 *)p2;
}

log_handle *log_create(cache *cc, log_config *lcfg, platform_heap_id hid) {
...
platform_status shard_log_iterator_init(cache *cc, shard_log_config *cfg,
                                        platform_heap_id hid, uint64 addr,
                                        uint64 magic,
  ...
  entry_size =
      sizeof(uint64) + cfg->data_cfg->key_size + cfg->data_cfg->message_size;
  itor->contents =
      TYPED_ARRAY_MALLOC(hid, itor->contents, itor->total * entry_size);
  ...
  while (extent_addr != 0 && cache_get_ref(cc, extent_addr) > 0) {
    ...
    for (i = 0; i < pages_per_extent; i++) {
      ...
      if (shard_log_valid(cfg, page, magic)) {
        ...
        memmove(itor->cursor, cursor, cfg->entries_per_page * entry_size);
  ...
  platform_sort_slow(itor->cursor, itor->total, entry_size, shard_log_compare,
                     NULL, temp);
...
void shard_log_iterator_deinit(platform_heap_id hid, shard_log_iterator *itor) {
  platform_free(hid, itor->contents);
}
...
void shard_log_iterator_get_curr(iterator *itorh, char **key, char **data,
                                 data_type *type) {
...
platform_status shard_log_iterator_at_end(iterator *itorh, bool *at_end) {
  shard_log_iterator *itor = (shard_log_iterator *)itorh;
...
platform_status shard_log_iterator_advance(iterator *itorh) {
  shard_log_iterator *itor = (shard_log_iterator *)itorh;
...
void shard_log_config_init(shard_log_config *log_cfg, data_config *data_cfg,
  ...
  log_entry_size = data_cfg->key_size + data_cfg->message_size + sizeof(uint64);
...
void shard_log_print(shard_log *log) {
  uint64 i, j;
  cache *cc = log->cc;
