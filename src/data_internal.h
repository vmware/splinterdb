                 ...
                 const slice   key2)
{
  ...
  return cfg->key_compare(cfg,
                          slice_length(key1), slice_data(key1),
                          slice_length(key2), slice_data(key2));
}

static inline message_type data_message_class(const data_config *cfg,
                                              const slice   raw_message)
{
   return cfg->message_class(cfg, slice_length(raw_message), slice_data(raw_message));
}
                                     ...
                                     slice        *new_raw_message)
{
  cfg->merge_tuples(cfg,
                    slice_length(key), slice_data(key),
                    slice_length(old_raw_message), slice_data(old_raw_message),
                    &new_raw_message->length, slice_data(*new_raw_message));
}
                                           ...
                                           slice        *oldest_raw_message)
{
   return cfg->merge_tuples_final(cfg,
                                  slice_length(key), slice_data(key),
                                  &oldest_raw_message->length, slice_data(*oldest_raw_message));
}
...
data_message_to_string(const data_config *cfg,
                       ...
                       size_t             size)
{
   cfg->message_to_string(cfg, slice_length(message), slice_data(message), str, size);
}
                                     ...
                                     void                         *new_raw_message)
{
  ...
  cfg->merge_tuples(cfg,
                    cfg->key_size, key,
                    cfg->message_size, old_raw_message,
                    &msglen, new_raw_message);
}
