...
#define clockcache_log(addr, entry, message, ...)                              \
  do {                                                                         \
    if (addr == TRACE_ADDR || entry == TRACE_ENTRY) {                          \
      platform_handle_log(cc->logfile, "(%lu) " message, platform_get_tid(),   \
                          ##__VA_ARGS__);                                      \
    }                                                                          \
  } while (0)
...
platform_status clockcache_alloc_extent(clockcache *cc, page_handle **page_arr,
                                        page_type type);
...
page_handle *clockcache_get(clockcache *cc, uint64 addr, bool blocking,
                            page_type type);
cache_async_result clockcache_get_async(clockcache *cc, uint64 addr,
                                        page_type type, cache_async_ctxt *ctxt);
void clockcache_async_done(clockcache *cc, page_type type,
...
void clockcache_share(clockcache *cc, page_handle *page_to_share,
...
void clockcache_page_sync(clockcache *cc, page_handle *page, bool is_blocking,
                          page_type type);
void clockcache_extent_sync(clockcache *cc, uint64 addr,
...
void clockcache_io_stats(clockcache *cc, uint64 *read_bytes,
                         uint64 *write_bytes);
...
#define CC_ALLOC_STATUS /* dirty */ (0 | CC_WRITELOCKED | CC_CLAIMED)
...
#define CC_WRITE_LOADING_STATUS                                                \
  (0 | CC_ACCESSED | CC_CLEAN | CC_LOADING | CC_WRITELOCKED)
...
static inline uint32 clockcache_set_flag(clockcache *cc, uint32 entry_number,
                                         uint32 flag) {
  return flag & __sync_fetch_and_or(&cc->entry[entry_number].status, flag);
}

static inline uint32 clockcache_clear_flag(clockcache *cc, uint32 entry_number,
                                           uint32 flag) {
  return flag & __sync_fetch_and_and(&cc->entry[entry_number].status, ~flag);
}

static inline uint32 clockcache_test_flag(clockcache *cc, uint32 entry_number,
                                          uint32 flag) {
  return flag & cc->entry[entry_number].status;
}

#ifdef RECORD_ACQUISITION_STACKS
static void clockcache_record_backtrace(clockcache *cc, uint32 entry_number) {
  int myhistindex =
...
static inline uint64 clockcache_multiply_by_page_size(clockcache *cc,
                                                      uint64 addr) {
  return addr << cc->cfg->log_page_size;
}

static inline uint64 clockcache_divide_by_page_size(clockcache *cc,
                                                    uint64 addr) {
  return addr >> cc->cfg->log_page_size;
}

static inline uint32 clockcache_lookup(clockcache *cc, uint64 addr) {
  uint64 lookup_no = clockcache_divide_by_page_size(cc, addr);
  return cc->lookup[lookup_no];
}

static inline clockcache_entry *clockcache_lookup_entry(clockcache *cc,
                                                        uint64 addr) {
  return &cc->entry[clockcache_lookup(cc, addr)];
}

static inline bool clockcache_pages_share_extent(clockcache *cc,
                                                 uint64 left_addr,
                                                 uint64 right_addr) {
  return left_addr / cc->cfg->extent_size == right_addr / cc->cfg->extent_size;
}

static inline clockcache_entry *clockcache_page_to_entry(clockcache *cc,
                                                         page_handle *page) {
  return (clockcache_entry *)((char *)page - offsetof(clockcache_entry, page));
}

static inline uint32 clockcache_page_to_entry_number(clockcache *cc,
                                                     page_handle *page) {
  return clockcache_page_to_entry(cc, page) - cc->entry;
}

static inline uint32 clockcache_data_to_entry_number(clockcache *cc,
                                                     char *data) {
  return clockcache_divide_by_page_size(cc, data - cc->data);
}

__attribute__((unused)) static inline clockcache_entry *
clockcache_data_to_entry(clockcache *cc, char *data) {
...
static inline uint32 clockcache_get_ref_internal(clockcache *cc,
                                                 uint32 entry_number) {
  return entry_number % cc->cfg->cacheline_capacity * PLATFORM_CACHELINE_SIZE +
         entry_number / cc->cfg->cacheline_capacity;
}

static inline uint16 clockcache_get_ref(clockcache *cc, uint32 entry_number,
                                        uint64 counter_no) {
...
static inline void clockcache_inc_ref(clockcache *cc, uint32 entry_number,
                                      threadid counter_no) {
  counter_no %= CC_RC_WIDTH;
  uint64 rc_number = clockcache_get_ref_internal(cc, entry_number);
  debug_assert(rc_number < cc->cfg->page_capacity);

  __attribute__((unused)) uint16 refcount = __sync_fetch_and_add(
      &cc->refcount[counter_no * cc->cfg->page_capacity + rc_number], 1);
  debug_assert(refcount != MAX_READ_REFCOUNT);
}

static inline void clockcache_dec_ref(clockcache *cc, uint32 entry_number,
                                      threadid counter_no) {
  counter_no %= CC_RC_WIDTH;
  uint64 rc_number = clockcache_get_ref_internal(cc, entry_number);
  debug_assert(rc_number < cc->cfg->page_capacity);

  __attribute__((unused)) uint16 refcount = __sync_fetch_and_sub(
      &cc->refcount[counter_no * cc->cfg->page_capacity + rc_number], 1);
  debug_assert(refcount != 0);
}

static inline uint8 clockcache_get_pin(clockcache *cc, uint32 entry_number) {
  uint64 rc_number = clockcache_get_ref_internal(cc, entry_number);
  debug_assert(rc_number < cc->cfg->page_capacity);
  return cc->pincount[rc_number];
}

static inline void clockcache_inc_pin(clockcache *cc, uint32 entry_number) {
  uint64 rc_number = clockcache_get_ref_internal(cc, entry_number);
  debug_assert(rc_number < cc->cfg->page_capacity);
...
static inline void clockcache_dec_pin(clockcache *cc, uint32 entry_number) {
  uint64 rc_number = clockcache_get_ref_internal(cc, entry_number);
  debug_assert(rc_number < cc->cfg->page_capacity);
  __attribute__((unused)) uint8 refcount =
...
void clockcache_assert_no_refs(clockcache *cc) {
  threadid i;
  volatile uint32 j;
...
void clockcache_assert_no_refs_and_pins(clockcache *cc) {
  threadid i;
  uint32 j;
...
void clockcache_assert_clean(clockcache *cc) {
  uint64 i;
...
static get_rc clockcache_try_get_read(clockcache *cc, uint32 entry_number,
                                      bool set_access) {
...
static get_rc clockcache_get_read(clockcache *cc, uint32 entry_number) {
  clockcache_record_backtrace(cc, entry_number);
...
static get_rc clockcache_try_get_claim(clockcache *cc, uint32 entry_number) {
  clockcache_record_backtrace(cc, entry_number);
...
static void clockcache_get_write(clockcache *cc, uint32 entry_number) {
  const threadid tid = platform_get_tid();
...
static get_rc clockcache_try_get_write(clockcache *cc, uint32 entry_number) {
  threadid thr_i;
...
static bool clockcache_touch_unless_evicted(clockcache *cc,
                                            uint32 entry_number) {
...
static inline bool clockcache_ok_to_writeback(clockcache *cc,
                                              uint32 entry_number,
                                              bool with_access) {
...
static inline bool clockcache_try_set_writeback(clockcache *cc,
                                                uint32 entry_number,
                                                bool with_access) {
  volatile uint32 *status = &cc->entry[entry_number].status;
  if (__sync_bool_compare_and_swap(status, CC_CLEANABLE1_STATUS,
                                   CC_WRITEBACK1_STATUS))
    return TRUE;

  if (with_access && __sync_bool_compare_and_swap(status, CC_CLEANABLE2_STATUS,
...
void clockcache_batch_start_writeback(clockcache *cc, uint64 batch,
                                      bool is_urgent) {
    ...
    if (clockcache_ok_to_writeback(cc, entry_no, is_urgent) &&
      ...
      *(clockcache **)req_metadata = cc;
      ...
      for (i = 0; i < req_count; i++) {
        ...
        clockcache_log_stream(addr, next_entry_no, "flush: entry %u addr %lu\n",
                              next_entry_no, addr);
        iovec[i].iov_base = next_entry->page.data;
      }

      status = io_write_async(cc->io, req, clockcache_write_callback, req_count,
                              first_addr);
      platform_assert_status_ok(status);
    }
  }
...
void clockcache_flush(clockcache *cc) {
  ...
  for (uint32 flush_hand = 0;
       flush_hand < cc->cfg->page_capacity / CC_ENTRIES_PER_BATCH; flush_hand++)
...
static void clockcache_try_evict(clockcache *cc, uint32 entry_number) {
...
void clockcache_evict_batch(clockcache *cc, uint32 batch) {
  debug_assert(cc != NULL);
  debug_assert(batch < cc->cfg->page_capacity / CC_ENTRIES_PER_BATCH);
...
int clockcache_evict_all(clockcache *cc, bool ignore_pinned_pages) {
  uint32 evict_hand;
  uint32 i;
...
void clockcache_move_hand(clockcache *cc, bool is_urgent) {
  const threadid tid = platform_get_tid();
...
uint32 clockcache_get_free_page(clockcache *cc, uint32 status, bool refcount,
                                bool blocking) {
  uint32 entry_no;
  uint64 num_passes = 0;
  const threadid tid = platform_get_tid();
...
void clockcache_config_init(clockcache_config *cache_cfg, uint64 page_size,
                            uint64 extent_size, uint64 capacity,
                            char *cache_logfile, uint64 use_stats) {
  int rc;
...
platform_status clockcache_init(clockcache *cc,          // OUT
                                ...
                                platform_module_id mid)  // IN
{
  ...
  /* Set up the entries */
  for (i = 0; i < cc->cfg->page_capacity; i++) {
    cc->entry[i].page.data = cc->data + clockcache_multiply_by_page_size(cc, i);
...
void clockcache_deinit(clockcache *cc) // IN/OUT
{
  platform_assert(cc != NULL);
...
clockcache_alloc_extent(clockcache *cc,
  ...
  for (i = 0; i < cc->cfg->pages_per_extent; i++) {
    entry_no = clockcache_get_free_page(cc, CC_ALLOC_STATUS,
                                        ...
                                        TRUE); // blocking
    entry = &cc->entry[entry_no];
    entry->page.disk_addr = base_addr + clockcache_multiply_by_page_size(cc, i);
    entry->type = type;
    if (cc->cfg->use_stats) {
...
void clockcache_try_dealloc_page(clockcache *cc, uint64 addr) {
  const threadid tid = platform_get_tid();
  while (TRUE) {
    ...
    clockcache_log(addr, entry_number, "dealloc (cached): entry %u addr %lu\n",
                   entry_number, addr);
...
static bool clockcache_get_internal(clockcache *cc,     // IN
                                    uint64 addr,        // IN
                                    bool blocking,      // IN
                                    page_type type,     // IN
                                    page_handle **page) // OUT
{
  ...
  __attribute__((unused)) uint64 base_addr = addr - addr % cc->cfg->extent_size;
  ...
  if (entry_number != CC_UNMAPPED_ENTRY) {
    ...
    clockcache_log(addr, entry_number,
                   ...
                   addr, clockcache_get_ref(cc, entry_number, tid));
    *page = &entry->page;
    return FALSE;
  }
  /*
   ...
   * do it.
   */
  if (!__sync_bool_compare_and_swap(&cc->lookup[lookup_no], CC_UNMAPPED_ENTRY,
                                    entry_number)) {
    clockcache_dec_ref(cc, entry_number, tid);
    entry->status = CC_FREE_STATUS;
    clockcache_log(addr, entry_number, "get abort: entry: %u addr: %lu\n",
                   entry_number, addr);
    return TRUE;
  }
  ...
  clockcache_log(addr, entry_number, "get (load): entry %u addr %lu\n",
                 entry_number, addr);
 ...
 *----------------------------------------------------------------------
 */
static void clockcache_read_async_callback(void *metadata, struct iovec *iovec,
                                           uint64 count,
                                           platform_status status) {
  ...
  clockcache_log(addr, entry_number, "async_get (load): entry %u addr %lu\n",
                 entry_number, addr);
  ctxt->status = status;
...
cache_async_result clockcache_get_async(clockcache *cc,         // IN
                                        uint64 addr,            // IN
                                        page_type type,         // IN
                                        cache_async_ctxt *ctxt) // IN
{
  ...
  if (!__sync_bool_compare_and_swap(&cc->lookup[lookup_no], CC_UNMAPPED_ENTRY,
    ...
    entry->status = CC_FREE_STATUS;
    clockcache_dec_ref(cc, entry_number, tid);
    clockcache_log(addr, entry_number, "get retry: entry: %u addr: %lu\n",
                   entry_number, addr);
    return async_locked;
  }
...
bool clockcache_claim(clockcache *cc, page_handle *page) {
  ...
  clockcache_record_backtrace(cc, entry_number);
  clockcache_log(page->disk_addr, entry_number, "claim: entry %u addr %lu\n",
...
void clockcache_unclaim(clockcache *cc, page_handle *page) {
  ...
  clockcache_record_backtrace(cc, entry_number);
  clockcache_log(page->disk_addr, entry_number, "unclaim: entry %u addr %lu\n",
                 entry_number, page->disk_addr);
...
void clockcache_lock(clockcache *cc, page_handle *page) {
  ...
  clockcache_record_backtrace(cc, entry_number);
  clockcache_log(page->disk_addr, entry_number, "lock: entry %u addr %lu\n",
                 entry_number, page->disk_addr);
  clockcache_get_write(cc, entry_number);
}
...
void clockcache_unlock(clockcache *cc, page_handle *page) {
  ...
  clockcache_record_backtrace(cc, entry_number);
  clockcache_log(page->disk_addr, entry_number, "unlock: entry %u addr %lu\n",
                 entry_number, page->disk_addr);
  __attribute__((unused)) uint32 was_writing =
...
void clockcache_page_sync(clockcache *cc, page_handle *page, bool is_blocking,
  ...
  if (!is_blocking) {
    ...
    *(clockcache **)req_metadata = cc;
    ...
    platform_assert_status_ok(status);
    clockcache_log(addr, entry_number, "page_sync write entry %u addr %lu\n",
                   entry_number, addr);
...
void clockcache_extent_sync(clockcache *cc, uint64 addr,
  ...
  if (req_count != 0) {
    __sync_fetch_and_add(pages_outstanding, req_count);
    status = io_write_async(cc->io, io_req, clockcache_sync_callback, req_count,
                            req_addr);
    platform_assert_status_ok(status);
  }
}
  ...
  if (full_extent) {
    ...
    *(clockcache **)req_metadata = cc;
    ...
    for (k = 0; k < req_count; k++) {
      ...
      clockcache_log(entry->page.disk_addr, entry_number,
                     "prefetch (load extent): addr %lu old_entry_number: %u "
                     "entry_number: %u lookup: %u\n",
                     entry->page.disk_addr, old_entry_number, entry_number,
                     clockcache_lookup(cc, lookup_no));
      iovec[k].iov_base = entry->page.data;
    }

    status =
        io_read_async(cc->io, req, clockcache_read_callback, req_count, addr);
    platform_assert_status_ok(status);
    return FALSE;
  } else {
    entry_number = clockcache_get_free_page(cc, CC_READ_LOADING_STATUS,
                                            FALSE, // refcount
                                            TRUE); // blocking
    clockcache_log(addr, entry_number, "prefetch (load): entry %u addr %lu\n",
                   entry_number, addr);
    entry = &cc->entry[entry_number];
    entry->page.disk_addr = addr;
    do {
  ...
  for (type = 0; type < NUM_PAGE_TYPES; type++) {
    ...
    if (allocs[type] != deallocs[type]) {
      platform_log("%s: allocs %lu deallocs %lu\n", page_type_strings[type],
                   allocs[PAGE_TYPE_TRUNK], deallocs[PAGE_TYPE_TRUNK]);
...
void clockcache_print_stats(clockcache *cc) {
  ...
  platform_log("---------------------------------------------------------------"
               "--------------------------------\n");
  platform_log("page type       |      trunk |     branch |   memtable |     "
               "filter |        log |       misc |\n");
  platform_log("----------------|------------|------------|------------|-------"
               "-----|------------|------------|\n");
  platform_log(
      "cache hits      | %10lu | %10lu | %10lu | %10lu | %10lu | %10lu |\n",
  ...
  platform_log(
      "cache misses    | %10lu | %10lu | %10lu | %10lu | %10lu | %10lu |\n",
  ...
  platform_log("cache miss time | " FRACTION_FMT(9, 2) "s | " FRACTION_FMT(9, 2) "s | " FRACTION_FMT(
                   9,
                   2) "s | " FRACTION_FMT(9,
                                          2) "s | " FRACTION_FMT(9,
                                                                 2) "s "
  ...
  platform_log(
      "pages allocated | %10lu | %10lu | %10lu | %10lu | %10lu | %10lu |\n",
  ...
  platform_log(
      "pages written   | %10lu | %10lu | %10lu | %10lu | %10lu | %10lu |\n",
  ...
  platform_log(
      "pages read      | %10lu | %10lu | %10lu | %10lu | %10lu | %10lu |\n",
  ...
  platform_log(
      "footprint       | %10lu | %10lu | %10lu | %10lu | %10lu | %10lu |\n",
      global_stats.page_allocs[PAGE_TYPE_TRUNK] -
...
allocator *clockcache_allocator(clockcache *cc) { return cc->al; }
