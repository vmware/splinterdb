...
typedef int (*log_write_fn)(log_handle *log, char *key, char *data,
                            uint64 generation);
...
static inline int log_write(log_handle *log, char *key, char *data,
                            uint64 generation) {
  return log->ops->write(log, key, data, generation);
}

static inline void log_release(log_handle *log) { log->ops->release(log); }

static inline uint64 log_addr(log_handle *log) { return log->ops->addr(log); }

static inline uint64 log_meta_addr(log_handle *log) {
  return log->ops->meta_addr(log);
}

static inline uint64 log_magic(log_handle *log) { return log->ops->magic(log); }

log_handle *log_create(cache *cc, log_config *cfg, platform_heap_id hid);
