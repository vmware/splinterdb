...
typedef int (*key_compare_fn)(const data_config *cfg, const void *key1,
                              const void *key2);
...
//    - get message_size (in case we need to memmove parts that don't get
//    updated)
...
typedef void (*merge_tuple_fn)(const data_config *cfg, const void *key,
                               const void *old_raw_message,
                               void *new_raw_message);

typedef void (*merge_tuple_final_fn)(const data_config *cfg, const void *key,
...
typedef void (*key_or_message_to_str_fn)(const data_config *cfg,
                                         const void *key_or_message, char *str,
                                         size_t max_len);
...
static inline int data_key_compare(const data_config *cfg, const void *key1,
                                   const void *key2) {
...
static inline void data_merge_tuples(const data_config *cfg, const void *key,
                                     const void *old_raw_message,
                                     void *new_raw_message) {
...
static inline void
data_clobber_message_with_range_delete(const data_config *cfg, const void *key,
                                       const void *message) {
  return cfg->clobber_message_with_range_delete(cfg, key, message);
}

static inline void data_key_to_string(const data_config *cfg, const void *key,
                                      char *str, size_t size) {
  cfg->key_to_string(cfg, key, str, size);
}

static inline void data_message_to_string(const data_config *cfg,
                                          const void *message, char *str,
                                          size_t size) {
...
static inline void data_key_copy(const data_config *cfg, void *dst,
                                 const void *src) {
