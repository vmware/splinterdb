...
platform_status splinter_range_delete(splinter_handle *spl, char *start_key,
                                      char *end_key);

platform_status splinter_lookup(splinter_handle *spl, char *key, char *data,
                                bool *found);
cache_async_result splinter_lookup_async(splinter_handle *spl, char *key,
...
static inline uint64 splinter_key_size(splinter_handle *spl) {
  return spl->cfg.data_cfg->key_size;
}

static inline uint64 splinter_message_size(splinter_handle *spl) {
  return spl->cfg.data_cfg->message_size;
}

static inline uint64 splinter_tuple_size(splinter_handle *spl) {
...
static inline void splinter_key_to_string(splinter_handle *spl, const char *key,
                                          char str[static 128]) {
  btree_key_to_string(&spl->cfg.btree_cfg, key, str);
}

static inline void splinter_message_to_string(splinter_handle *spl,
                                              const char *data,
                                              char str[static 128]) {
  btree_message_to_string(&spl->cfg.btree_cfg, data, str);
}

static inline void splinter_async_ctxt_init(splinter_async_ctxt *ctxt,
...
uint64 splinter_pivot_message_size();

uint64 splinter_trunk_hdr_size();
...
size_t splinter_get_scratch_size();
