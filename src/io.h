...
typedef void (*io_callback_fn)(void *metadata, struct iovec *iovec,
                               uint64 count, platform_status status);

typedef platform_status (*io_read_fn)(io_handle *io, void *buf, uint64 bytes,
                                      uint64 addr);
typedef platform_status (*io_write_fn)(io_handle *io, void *buf, uint64 bytes,
                                       uint64 addr);
...
typedef platform_status (*io_read_async_fn)(io_handle *io, io_async_req *req,
                                            io_callback_fn callback,
                                            uint64 count, uint64 addr);
typedef platform_status (*io_write_async_fn)(io_handle *io, io_async_req *req,
                                             io_callback_fn callback,
                                             uint64 count, uint64 addr);
...
platform_status io_handle_init(platform_io_handle *ioh, io_config *cfg,
                               platform_heap_handle hh, platform_heap_id hid);

void io_handle_deinit(platform_io_handle *ioh);

static inline platform_status io_read(io_handle *io, void *buf, uint64 bytes,
                                      uint64 addr) {
  return io->ops->read(io, buf, bytes, addr);
}

static inline platform_status io_write(io_handle *io, void *buf, uint64 bytes,
                                       uint64 addr) {
  return io->ops->write(io, buf, bytes, addr);
}

static inline io_async_req *io_get_async_req(io_handle *io, bool blocking) {
  return io->ops->get_async_req(io, blocking);
}

static inline struct iovec *io_get_iovec(io_handle *io, io_async_req *req) {
  return io->ops->get_iovec(io, req);
}

static inline void *io_get_metadata(io_handle *io, io_async_req *req) {
  return io->ops->get_metadata(io, req);
}

static inline platform_status io_read_async(io_handle *io, io_async_req *req,
                                            io_callback_fn callback,
                                            uint64 count, uint64 addr) {
  return io->ops->read_async(io, req, callback, count, addr);
}

static inline platform_status io_write_async(io_handle *io, io_async_req *req,
                                             io_callback_fn callback,
                                             uint64 count, uint64 addr) {
  return io->ops->write_async(io, req, callback, count, addr);
}

static inline void io_cleanup(io_handle *io, uint64 count) {
  return io->ops->cleanup(io, count);
}

// Guarantees all in-flight IOs are complete before return
static inline void io_cleanup_all(io_handle *io) {
  return io->ops->cleanup_all(io);
}

static inline void io_thread_register(io_handle *io) {
  if (io->ops->thread_register) {
    return io->ops->thread_register(io);
  }
}

static inline bool io_max_latency_elapsed(io_handle *io, timestamp ts) {
...
static inline void io_config_init(io_config *io_cfg, uint64 page_size,
                                  uint64 extent_size, int flags, uint32 perms,
                                  uint64 async_queue_depth, char *io_filename) {
