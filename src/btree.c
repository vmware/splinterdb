...
static inline uint64 btree_root_to_meta_addr(cache *cc, btree_config *cfg,
                                             uint64 root_addr,
                                             uint64 meta_page_no) {
  return root_addr + (meta_page_no + 1) * cfg->page_size;
}

void btree_iterator_get_curr(iterator *itor, char **key, char **data,
                             data_type *type);
...
uint64 btree_alloc(mini_allocator *mini, uint64 height, char *key,
                   uint64 *next_extent) {
  return mini_allocator_alloc(mini, height, key, next_extent);
}

static inline void btree_merge_tuples(btree_config *cfg, const char *key,
                                      const char *old_data, char *new_data) {
...
void btree_node_get(cache *cc, btree_config *cfg, btree_node *node,
                    page_type type) {
...
bool btree_node_claim(cache *cc,         // IN
                      btree_config *cfg, // IN
                      btree_node *node)  // IN
{
...
void btree_node_lock(cache *cc,         // IN
                     btree_config *cfg, // IN
                     btree_node *node)  // IN
{
...
void btree_node_unlock(cache *cc,         // IN
                       btree_config *cfg, // IN
                       btree_node *node)  // IN
{
...
void btree_node_unclaim(cache *cc,         // IN
                        btree_config *cfg, // IN
                        btree_node *node)  // IN
{
...
void btree_node_unget(cache *cc,         // IN
                      btree_config *cfg, // IN
                      btree_node *node)  // IN
{
...
static inline void btree_node_full_unlock(cache *cc,         // IN
                                          btree_config *cfg, // IN
                                          btree_node *node)  // IN
{
...
static inline void btree_node_get_from_cache_ctxt(btree_config *cfg,      // IN
...
static inline uint8 *btree_get_table(btree_config *cfg, btree_node *node) {
  return (uint8 *)node->hdr + sizeof(btree_hdr);
}

static inline uint16 btree_num_entries(btree_config *cfg, btree_node *node) {
  return node->hdr->num_entries;
}

static inline void btree_inc_num_entries(btree_config *cfg, btree_node *node) {
  node->hdr->num_entries++;
}

static inline uint16 btree_height(btree_config *cfg, btree_node *node) {
  return node->hdr->height;
}

static inline bool btree_is_packed(btree_config *cfg, btree_node *node) {
  return node->hdr->is_packed;
}

static inline bool btree_node_is_full(btree_config *cfg, btree_node *node) {
...
static inline char *btree_get_pivot(btree_config *cfg, btree_node *node,
                                    uint32 k) {
...
static inline uint64 btree_pivot_addr(btree_config *cfg, btree_node *node,
                                      uint32 k) {
  return *(uint64 *)(btree_get_pivot(cfg, node, k) + btree_key_size(cfg));
}

static inline void btree_set_pivot_key_and_addr(btree_config *cfg,
                                                btree_node *node, uint32 k,
 ...
 */
static inline uint8 btree_shift_unpacked_pivots(btree_config *cfg,
                                                btree_node *node, uint32 idx) {
  ...
  size_t bytes_to_shift = (btree_num_entries(cfg, node) - idx) * sizeof(*table);
...
static inline char *btree_get_tuple(btree_config *cfg, btree_node *node,
                                    uint32 k) {
  btree_hdr *hdr = node->hdr;
  uint8 idx;
  uint8 *table;
...
static inline char *btree_get_data(btree_config *cfg, btree_node *node,
                                   uint32 k) {
 ...
 *
 *      in the first step, computes the largest power of 2 less than
 *num_entries, then starts the binary search on the two overlapping ranges of
 *that size which cover all the indices. E.g. for 7, the power of 2 is 4 and the
 *ranges are: (1 2 3 (4) 5 6 7)
 *
...
static inline uint32 round_down_logarithm(uint32 size) {
  if (size <= 1)
    return 0;
  return (8 * sizeof(uint32)) - __builtin_clz(size - 1);
}

static inline void btree_update_lowerbound(uint16 *lo, uint16 *mid, int cmp,
                                           lookup_type comp) {
  switch (comp) {
  case less_than:
...
static inline uint32 btree_find_pivot(btree_config *cfg, btree_node *node,
                                      const char *key, lookup_type comp) {
 ...
 */
static inline void btree_add_pivot_at_pos(btree_config *cfg,
                                          btree_node *parent_node,
                                          btree_node *child_node, uint16 pos,
                                          bool is_new_root) {
...
static inline uint32 btree_find_tuple(btree_config *cfg, btree_node *node,
                                      const char *key, lookup_type comp) {
...
btree_shift_packed_tuples(btree_config *cfg, btree_node *node, uint32 idx,
                          uint32 shift) {
  debug_assert(btree_is_packed(cfg, node));
  debug_assert(btree_height(cfg, node) == 0);
 ...
 * this shift only affects the redirection table
 */
static inline uint8 btree_shift_unpacked_tuples(btree_config *cfg,
                                                btree_node *node, uint32 idx,
                                                uint32 shift) {
  debug_assert(!btree_is_packed(cfg, node));
  debug_assert(btree_height(cfg, node) == 0);
 ...
 */
static inline bool btree_add_tuple(btree_config *cfg, btree_scratch *scratch,
                                   btree_node *node, const char *key,
                                   const char *data, uint64 *generation) {
  debug_assert(btree_height(cfg, node) == 0);
  ...
  *generation = node->hdr->generation++;
  ...
  TRACESplinter(1, SPLINTERTraceBTreeAddTuple, idx, node->hdr->num_entries,
                free_entry);

  return TRUE;
}

static inline void btree_add_tuple_at_pos(btree_config *cfg, btree_node *node,
                                          char *key, char *data, uint16 pos) {
...
static inline bool btree_addrs_share_extent(btree_config *cfg, uint64 left_addr,
                                            uint64 right_addr) {
  return right_addr / cfg->extent_size == left_addr / cfg->extent_size;
}

static inline uint64 btree_page_size(btree_config *cfg) {
  return cfg->page_size;
}

static inline uint64 btree_get_extent_base_addr(btree_config *cfg,
                                                btree_node *node) {
  return node->addr / cfg->extent_size * cfg->extent_size;
}

__attribute__((unused)) static inline bool
btree_range_valid(cache *cc, btree_config *cfg, uint64 root_addr,
                  page_type type, char *min_key, char *max_key) {
...
void btree_split_node(cache *cc,              // IN
                      btree_config *cfg,      // IN
                      btree_node *left_node,  // IN
                      btree_node *right_node) // IN
{
  uint16 target_right_entries = btree_num_entries(cfg, left_node) / 2;
  uint16 target_left_entries =
      btree_num_entries(cfg, left_node) - target_right_entries;
  ...
  const size_t suffix = btree_page_size(cfg) - header_to_table -
                        target_left_entries - target_right_entries;
...
void btree_add_shared_pivot(cache *cc, btree_config *cfg, mini_allocator *mini,
                            btree_node *parent, btree_node *child,
...
uint64 btree_init(cache *cc, btree_config *cfg, mini_allocator *mini,
                  bool is_packed) {
 ...
 */
bool btree_should_zap_dec_ref(cache *cc, btree_config *cfg, uint64 root_addr,
                              page_type type) {
...
void btree_inc_range(cache *cc, btree_config *cfg, uint64 root_addr,
                     const char *start_key, const char *end_key) {
  uint64 meta_page_addr = btree_root_to_meta_addr(cc, cfg, root_addr, 0);
  if (start_key != NULL && end_key != NULL) {
    debug_assert(btree_key_compare(cfg, start_key, end_key) < 0);
  }
  mini_allocator_inc_range(cc, cfg->data_cfg, PAGE_TYPE_BRANCH, meta_page_addr,
                           start_key, end_key);
}

bool btree_zap_range(cache *cc, btree_config *cfg, uint64 root_addr,
                     const char *start_key, const char *end_key,
                     page_type type) {
...
bool btree_zap(cache *cc, btree_config *cfg, uint64 root_addr, page_type type) {
  return btree_zap_range(cc, cfg, root_addr, NULL, NULL, type);
}

page_handle *btree_blind_inc(cache *cc, btree_config *cfg, uint64 root_addr,
                             page_type type) {
...
void btree_blind_zap(cache *cc, btree_config *cfg, page_handle *meta_page,
                     page_type type) {
...
platform_status btree_insert(cache *cc,              // IN
                             btree_config *cfg,      // IN
...
start_over:
  ...
  for (height = btree_height(cfg, &root_node); height != 0; height--) {
    ...
    if (btree_node_is_full(cfg, &child_node)) {
      ...
      debug_assert(btree_pivot_addr(cfg, &parent_node, child_idx) ==
                   child_node.addr);
...
btree_lookup_node(cache *cc,             // IN
                  ...
                  page_type type,        // IN
                  uint64 *out_rank,      // OUT returns the rank of the out_node
                                         // among nodes of its height
                                        ...
                                        // stop_at_height in which key was found
{
  ...
  uint64 ppi = btree_is_packed(cfg, &node) ? cfg->pivots_per_packed_index
...
inline void btree_lookup_with_ref(cache *cc,         // IN
                                  btree_config *cfg, // IN
                                  uint64 root_addr,  // IN
                  ...
                  bool *found)       // OUT
{
  btree_node node;
  char *data;
  btree_lookup_with_ref(cc, cfg, root_addr, &node, PAGE_TYPE_BRANCH, key, &data,
                        found);
  if (*found) {
...
cache_async_result btree_lookup_async(cache *cc,              // IN
                                      btree_config *cfg,      // IN
                                      uint64 root_addr,       // IN
                                      char *key,              // IN
                                      char *data_out,         // OUT
...
static inline void btree_async_set_state(btree_async_ctxt *ctxt,
...
static void btree_async_callback(cache_async_ctxt *cache_ctxt) {
...
cache_async_result btree_lookup_async_with_ref(cache *cc,              // IN
                                               btree_config *cfg,      // IN
                                               uint64 root_addr,       // IN
                                               ...
                                               btree_async_ctxt *ctxt) // IN
{
  ...
  do {
    ...
    case btree_async_state_get_node: {
      ...
      res = cache_get_async(cc, ctxt->child_addr, PAGE_TYPE_BRANCH, cache_ctxt);
      switch (res) {
      case async_locked:
      case async_no_reqs:
    ...
    case btree_async_state_get_index_complete: {
      ...
      uint16 child_idx = btree_find_pivot(cfg, node, key, less_than_or_equal);
    ...
    case btree_async_state_get_leaf_complete: {
      const uint16 idx =
          btree_find_tuple(cfg, node, key, greater_than_or_equal);
...
void btree_iterator_get_curr(iterator *base_itor, char **key, char **data,
                             data_type *type) {
  ...
  if (itor->curr.addr != itor->end.addr || itor->idx != itor->end_idx) {
    if (itor->idx == itor->curr.hdr->num_entries &&
      ...
      platform_log("btree_iterator bad at_end %lu curr %lu idx %u "
                   "end %lu end_idx %u\n",
                   itor->root_addr, itor->start_addr, itor->start_idx,
...
static bool btree_iterator_is_at_end(btree_iterator *itor) {
...
static bool btree_iterator_is_last_tuple(btree_iterator *itor) {
  debug_assert(!itor->at_end);
  ...
  if (itor->is_live) {
    ...
    if (itor->max_key != NULL) {
      ...
      if (itor->idx + 1 == itor->curr.hdr->num_entries) {
        ...
        next_key = btree_get_tuple(cfg, &itor->curr, itor->idx + 1);
      }
...
static void btree_iterator_set_curr_key_and_data(btree_iterator *itor) {
  debug_assert(itor != NULL);
  debug_assert(itor->curr.hdr != NULL);
  debug_assert(!itor->at_end);
  debug_assert(itor->idx < itor->curr.hdr->num_entries);
...
debug_btree_iterator_validate_next_tuple(debug_only btree_iterator *itor) {
#if SPLINTER_DEBUG
  btree_config *cfg = itor->cfg;
  debug_assert(btree_key_compare(cfg, itor->debug_prev_key, itor->curr_key) <
               0);
  if (itor->max_key) {
  ...
  if (itor->cfg->type == data_type_range) {
    ...
    if (itor->max_key) {
      debug_assert(btree_key_compare(cfg, itor->curr_data, itor->max_key) <= 0);
    }
  }
#endif
}
...
static void btree_iterator_clamp_end(btree_iterator *itor) {
  if (1 && itor->height == 0 && itor->cfg->type == data_type_range &&
...
platform_status btree_iterator_advance(iterator *base_itor) {
  debug_assert(base_itor != NULL);
  btree_iterator *itor = (btree_iterator *)base_itor;
 ...
 *-----------------------------------------------------------------------------
 */
void btree_iterator_init(cache *cc, btree_config *cfg, btree_iterator *itor,
                         uint64 root_addr, page_type page_type,
                         const char *min_key, const char *max_key,
                         bool do_prefetch, bool is_live, uint32 height,
                         data_type data_type) {
  ...
  if (!is_live && max_key != NULL) {
    ...
    if (height == 0) {
      itor->end_idx =
          btree_find_tuple(cfg, &itor->end, max_key, greater_than_or_equal);
    } else {
      itor->end_idx =
          btree_find_pivot(cfg, &itor->end, max_key, greater_than_or_equal);
    }

    debug_assert(itor->end_idx == itor->end.hdr->num_entries ||
                 ((height == 0) &&
                  btree_key_compare(
                      cfg, max_key,
                      btree_get_tuple(cfg, &itor->end, itor->end_idx)) <= 0) ||
                 ((height != 0) &&
                  btree_key_compare(
                      cfg, max_key,
    ...
    debug_assert(itor->end_idx == itor->end.hdr->num_entries ||
                 ...
                 ((height == 0) &&
                  btree_key_compare(cfg, max_key,
                                    btree_get_tuple(cfg, &itor->end,
                                                    itor->end_idx - 1)) > 0) ||
                 ((height != 0) &&
                  btree_key_compare(cfg, max_key,
                                    btree_get_pivot(cfg, &itor->end,
                                                    itor->end_idx - 1)) > 0));
  ...
  if (height == 0) {
    ...
    itor->idx =
        btree_find_tuple(cfg, &itor->curr, min_key, greater_than_or_equal);
    ...
    itor->idx =
        btree_find_pivot(cfg, &itor->curr, min_key, greater_than_or_equal);
  }
    ...
    if (itor->do_prefetch && btree_is_packed(cfg, &itor->curr)) {
      ...
      if (next_extent_addr != 0   // prefetch if not last extent
          && (itor->end.addr == 0 // and either no upper bound
              || !btree_addrs_share_extent(cfg, itor->curr.addr,
                                           itor->end.addr))) {
...
void btree_iterator_deinit(btree_iterator *itor) {
  debug_assert(itor != NULL);
  if (itor->curr.addr != 0) {
...
static inline void btree_pack_node_init_hdr(btree_node *node,
                                            uint64 next_extent, uint8 height,
                                            bool is_packed) {
...
static inline void btree_pack_setup(btree_pack_req *req,
...
static inline void btree_pack_loop(btree_pack_internal *tree, // IN/OUT
                                   char *key,                 // IN
                                   char *data,                // IN
                                   bool *at_end)              // IN/OUT
{
  if (tree->idx[0] == tree->cfg->tuples_per_packed_leaf) {
    ...
    tree->new_edge.addr = btree_alloc(&tree->mini, 0, key, &tree->next_extent);
    ...
    __attribute__((unused)) bool success =
        btree_node_claim(tree->cc, tree->cfg, &tree->new_edge);
    ...
    for (i = 1; tree->idx[i] == tree->cfg->pivots_per_packed_index; i++) {
      ...
      btree_add_pivot_at_pos(tree->cfg, &tree->edge[i], &tree->edge[i - 1], 0,
                             FALSE);
    ...
    // if (idx[0] != 0) {
    //   int comp = btree_key_compare(cfg, btree_get_tuple(cfg, &edge[0], idx[0]
    //   - 1), key); if (comp >= 0) {
    //      char key_str[128], last_key_str[128];
    //      btree_key_to_string(cfg, key, key_str);
    //      btree_key_to_string(cfg, btree_get_tuple(cfg, &edge[0], idx[0] - 1),
    //      last_key_str); platform_log("btree_pack OOO keys: \n%s \n%s\n%d\n",
...
static inline void btree_pack_post_loop(btree_pack_internal *tree) {
  cache *cc = tree->cc;
  btree_config *cfg = tree->cfg;
  ...
  for (uint16 i = 0; i <= tree->height; i++) {
    ...
    for (uint64 addr = btree_get_extent_base_addr(cfg, &tree->edge[i]);
         addr != tree->edge[i].addr; addr += btree_page_size(cfg)) {
...
platform_status btree_pack(btree_pack_req *req) {
  btree_pack_internal tree;
  ZERO_STRUCT(tree);
...
platform_status branch_pack(platform_heap_id hid, branch_pack_req *req) {
  ...
  platform_assert(
      IMPLIES(req->point_req.num_tuples == 0, req->point_req.root_addr == 0));
  btree_pack_post_loop(range_tree);
  platform_assert(
      IMPLIES(req->range_req.num_tuples == 0, req->range_req.root_addr == 0));
 ...
 */
static inline uint64 btree_get_rank(cache *cc, btree_config *cfg,
                                    uint64 root_addr, const char *key) {
  btree_node leaf;
  uint64 leaf_rank;
...
uint64 btree_count_in_range(cache *cc, btree_config *cfg, uint64 root_addr,
                            const char *min_key, const char *max_key) {
...
uint64 btree_count_in_range_by_iterator(cache *cc, btree_config *cfg,
                                        uint64 root_addr, const char *min_key,
                                        const char *max_key) {
...
void btree_print_locked_node(cache *cc, btree_config *cfg, btree_node *node,
...
void btree_print_node(cache *cc, btree_config *cfg, btree_node *node,
                      platform_stream_handle stream) {
...
void btree_print_subtree(cache *cc, btree_config *cfg, uint64 addr,
                         platform_stream_handle stream) {
  btree_node node;
  node.addr = addr;
...
void btree_print_tree(cache *cc, btree_config *cfg, uint64 root_addr) {
  platform_open_log_stream();
  btree_print_subtree(cc, cfg, root_addr, stream);
  platform_close_log_stream(PLATFORM_DEFAULT_LOG_HANDLE);
}

void btree_print_tree_stats(cache *cc, btree_config *cfg, uint64 addr) {
  btree_node node;
  node.addr = addr;
  btree_node_get(cc, cfg, &node, PAGE_TYPE_BRANCH);
...
uint64 btree_space_use_in_range(cache *cc, btree_config *cfg, uint64 root_addr,
                                page_type type, const char *start_key,
                                const char *end_key) {
  uint64 meta_head = btree_root_to_meta_addr(cc, cfg, root_addr, 0);
  uint64 extents_used = mini_allocator_count_extents_in_range(
      cc, cfg->data_cfg, type, meta_head, start_key, end_key);
  return extents_used * cfg->extent_size;
}

bool btree_verify_node(cache *cc, btree_config *cfg, uint64 addr,
                       page_type type, bool is_left_edge) {
  btree_node node;
  node.addr = addr;
  debug_assert(type == PAGE_TYPE_BRANCH || type == PAGE_TYPE_MEMTABLE);
  btree_node_get(cc, cfg, &node, type);
    ...
    if (node.hdr->height == 0) {
      ...
      if (child.hdr->height == 0) {
        ...
        if (idx != btree_num_entries(cfg, &node) - 1 &&
            btree_key_compare(
                cfg, btree_get_pivot(cfg, &node, idx + 1),
                btree_get_tuple(cfg, &child,
                                btree_num_entries(cfg, &child) - 1)) < 0) {
          platform_log_stream("child tuple larger than parent bound\n");
          platform_log_stream("addr: %lu idx %u\n", node.addr, idx);
          platform_log_stream("child addr: %lu idx %u\n", child.addr, idx);
          btree_print_locked_node(cc, cfg, &node, PLATFORM_ERR_LOG_HANDLE);
          btree_print_locked_node(cc, cfg, &child, PLATFORM_ERR_LOG_HANDLE);
          platform_assert(0);
          btree_node_unget(cc, cfg, &child);
          btree_node_unget(cc, cfg, &node);
          goto out;
...
bool btree_verify_tree(cache *cc, btree_config *cfg, uint64 addr,
                       page_type type) {
  return btree_verify_node(cc, cfg, addr, type, TRUE);
}

void btree_print_lookup(cache *cc,         // IN
                        btree_config *cfg, // IN
                        uint64 root_addr,  // IN
                        page_type type,    // IN
                        char *key)         // IN
...
uint64 btree_extent_count(cache *cc, btree_config *cfg, uint64 root_addr) {
  uint64 meta_head = btree_root_to_meta_addr(cc, cfg, root_addr, 0);
...
void btree_config_init(btree_config *btree_cfg, data_config *data_cfg,
                       data_type type, uint64 rough_count_height,
                       uint64 page_size, uint64 extent_size) {
