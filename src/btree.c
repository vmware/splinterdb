                        ...
                        lookup_result               *old_result)
{
   ...
   return lookup_result_update(
      old_result, leaf_entry_key(entry), leaf_entry_message(cc, entry));
}
                                   ...
                                   leaf_incorporate_spec *spec)
{
   ...
   if (!found) {
      ...
      if (message_isblob(msg)) {
         ...
         rc = message_clone(&btree_blob_cfg, cc, mini, msg, &spec->msg.ma);
      } else if (MAX_INLINE_MESSAGE_SIZE(btree_page_size(cfg))
                 < message_length(msg))
      {
         spec->use_msg = FALSE;
         merge_accumulator_init(&spec->msg.ma, heap_id);
         rc = message_to_blob(&btree_blob_cfg, cc, mini, msg, &spec->msg.ma);
      } else {
         spec->use_msg = TRUE;
         spec->msg.msg = msg;
      }
                 ...
                 < merge_accumulator_length(&spec->msg.ma))
      {
         rc = merge_accumulator_convert_to_blob(
            &btree_blob_cfg, cc, mini, &spec->msg.ma);
      }
   }
                       ...
                       uint64                *generation) // OUT
{
   ...
   platform_status rc =
      btree_record_old_result(cfg, cc, child->hdr, spec, old_result);
   if (!SUCCESS(rc)) {
      if (child_next.addr != 0) {
...
start_over:
   ...
   if (btree_can_perform_leaf_incorporate_spec(cfg, child_node.hdr, &spec)) {
      ...
      btree_node_lock(cc, cfg, &child_node);
      rc = btree_record_old_result(cfg, cc, child_node.hdr, &spec, old_result);
      if (!SUCCESS(rc)) {
                ...
                message         msg)               // IN
{
   ...
   if (message_isblob(msg)) {
      rc = message_clone(&btree_blob_cfg, req->cc, &req->mini, msg, &req->ma);
      if (!SUCCESS(rc)) {
         return rc;
      }
      msg = merge_accumulator_to_message(&req->ma);
              ...
              < message_length(msg))
   {
      rc = message_to_blob(&btree_blob_cfg, req->cc, &req->mini, msg, &req->ma);
      if (!SUCCESS(rc)) {
         return rc;
      }
      msg = merge_accumulator_to_message(&req->ma);
