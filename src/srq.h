...
static inline void srq_init(srq *queue,
...
static inline void srq_deinit(srq *queue) {
  platform_mutex_destroy(&queue->mutex);
}

static inline int64 srq_parent(int64 pos) {
  debug_assert(pos >= 0);
  return (pos - 1) / 2;
}

static inline int64 srq_lchild(int64 pos) {
  debug_assert(pos >= 0);
  return 2 * pos + 1;
}

static inline int64 srq_rchild(int64 pos) {
  debug_assert(pos >= 0);
  return 2 * pos + 2;
}
 ...
 */
static inline bool srq_has_priority(srq *queue, int64 lpos, int64 rpos) {
  debug_assert(lpos >= 0);
  debug_assert(rpos >= 0);
 ...
 */
static inline void srq_update_index(srq *queue, int64 pos) {
  debug_assert(pos >= 0);
  srq_data *data = &queue->heap[pos];
  queue->index[data->idx] = pos;
}

static inline void srq_swap(srq *queue, int64 lpos, int64 rpos) {
  debug_assert(lpos >= 0);
  debug_assert(rpos >= 0);
...
static inline void srq_move_tail_to_pos(srq *queue, int64 pos) {
  debug_assert(pos >= 0);
...
static inline void srq_rebalance_up(srq *queue, int64 pos) {
  debug_assert(pos >= 0);
...
static inline void srq_rebalance_down(srq *queue, uint64 pos) {
  debug_assert(pos >= 0);
  debug_assert(0 || (1 && queue->num_entries == 0 && pos == 0) ||
               pos < queue->num_entries);
  while (0 ||
...
static inline uint64 srq_get_new_index(srq *queue) {
...
static inline bool srq_verify(srq *queue);

static inline void srq_print(srq *queue);

static inline uint64 srq_insert(srq *queue, srq_data new_data) {
  srq_print(queue);
...
static inline bool srq_data_found(srq_data *data) {
 ...
 */
static inline srq_data srq_extract_max(srq *queue) {
  srq_print(queue);
  platform_mutex_lock(&queue->mutex);
  if (queue->num_entries == 0) {
...
static inline srq_data srq_delete(srq *queue, int64 idx) {
  srq_print(queue);
  platform_mutex_lock(&queue->mutex);
...
static inline void srq_update(srq *queue, int64 idx, uint32 new_priority) {
  platform_mutex_lock(&queue->mutex);
  int64 pos = queue->index[idx];
  platform_assert(pos != SRQ_INDEX_AVAILABLE);
...
static inline void srq_print(srq *queue) {
  ...
  for (uint64 i = 0; i < queue->num_entries; i++) {
    ...
    platform_log("%4lu: %12lu-%lu %8lu", i, data.addr, data.pivot_generation,
                 data.priority);
    ...
    if (i != 0) {
      ...
      platform_log("parent %4lu: %12lu-%lu %8lu", srq_parent(i), data.addr,
                   data.pivot_generation, data.priority);
    ...
    if (srq_lchild(i) < queue->num_entries) {
      ...
      platform_log("lchild %4lu: %12lu-%lu %8lu", srq_lchild(i), data.addr,
                   data.pivot_generation, data.priority);
    }
    if (srq_rchild(i) < queue->num_entries) {
      data = queue->heap[srq_rchild(i)];
      platform_log(" rchild %4lu: %12lu-%lu %8lu", srq_rchild(i), data.addr,
                   data.pivot_generation, data.priority);
    }
    if (queue->num_entries != 1) {
      platform_log(")");
