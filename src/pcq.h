 ...
 */
static inline pcq *pcq_alloc(platform_heap_id hid, size_t num_elems) {
  pcq *q;
 ...
 */
static inline uint32 pcq_count(const pcq *q) { return q->tail.v - q->head.v; }

// Return TRUE if PCQ is empty, FALSE otherwise
static inline bool pcq_is_empty(const pcq *q) { return q->head.v == q->tail.v; }

// Return TRUE if PCQ is full, FALSE otherwise
static inline bool pcq_is_full(const pcq *q) {
  return pcq_count(q) == q->num_elems;
}

// Deallocate a PCQ
static inline void pcq_free(platform_heap_id hid, pcq *q) {
  platform_free(hid, q);
}

// Enqueue an elem to a PCQ. Element must not be NULL
static inline void pcq_enqueue(pcq *q, void *elem) {
 ...
 * not done here.
 */
static inline platform_status pcq_dequeue(pcq *q,      // IN
                                          void **elem) // OUT
{
  if (pcq_is_empty(q)) {
