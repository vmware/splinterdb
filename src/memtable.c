...
bool memtable_is_full(memtable_context *ctxt) {
...
bool memtable_is_empty(memtable_context *ctxt) { return ctxt->num_tuples == 0; }

static inline void memtable_clear_num_tuples(memtable_context *ctxt) {
  ZERO_ARRAY(ctxt->thread_num_tuples);
  ctxt->num_tuples = 0;
}

static inline void memtable_process(memtable_context *ctxt, uint64 generation) {
 ...
 */
static inline void memtable_add_tuple(memtable_context *ctxt) {
  const threadid tid = platform_get_tid();
...
platform_status memtable_insert(memtable_context *ctxt, memtable *mt,
  ...
  bool was_unique;
  platform_status rc =
      btree_insert(ctxt->cc, ctxt->cfg.btree_cfg, ctxt->scratch, mt->root_addr,
                   &mt->mini, key, data, leaf_generation, &was_unique);
  if (!SUCCESS(rc)) {
    return rc;
  }
...
void memtable_unget_insert_lock(memtable_context *ctxt,
...
page_handle *memtable_try_get_lookup_lock(memtable_context *ctxt) {
  return cache_get(ctxt->cc, ctxt->lookup_lock_addr, FALSE,
                   PAGE_TYPE_LOCK_NO_DATA);
}

void memtable_unget_lookup_lock(memtable_context *ctxt,
                                page_handle *lock_page) {
  cache_unget(ctxt->cc, lock_page);
}
