...
typedef platform_status (*alloc_extent_fn)(allocator *al, uint64 *addr_arr);
...
static inline platform_status allocator_alloc_extent(allocator *al,
                                                     uint64 *addr) {
  return al->ops->alloc_extent(al, addr);
}

static inline uint8 allocator_inc_refcount(allocator *al, uint64 addr) {
  return al->ops->inc_refcount(al, addr);
}

static inline uint8 allocator_dec_refcount(allocator *al, uint64 addr) {
  return al->ops->dec_refcount(al, addr);
}

static inline uint8 allocator_get_refcount(allocator *al, uint64 addr) {
  return al->ops->get_refcount(al, addr);
}

static inline uint64 allocator_get_capacity(allocator *al) {
  return al->ops->get_capacity(al);
}

static inline platform_status allocator_get_super_addr(allocator *al,
                                                       allocator_root_id spl_id,
                                                       uint64 *addr) {
...
allocator_alloc_super_addr(allocator *al, allocator_root_id spl_id,
                           uint64 *addr) {
  return al->ops->alloc_super_addr(al, spl_id, addr);
}

static inline uint64 allocator_in_use(allocator *al) {
  return al->ops->in_use(al);
}

static inline uint64 allocator_max_allocated(allocator *al) {
  return al->ops->max_allocated(al);
}

static inline uint64 allocator_extent_size(allocator *al) {
  return al->ops->get_extent_size(al);
}

static inline uint64 allocator_page_size(allocator *al) {
  return al->ops->get_page_size(al);
}

static inline void allocator_print_allocated(allocator *al) {
  return al->ops->print_allocated(al);
}

static inline void allocator_remove_super_addr(allocator *al,
                                               allocator_root_id spl_id) {
