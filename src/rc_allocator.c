...
platform_status rc_allocator_alloc_super_addr(rc_allocator *al,
                                              allocator_root_id spl_id,
                                              uint64 *addr);
void rc_allocator_remove_super_addr(rc_allocator *al, allocator_root_id spl_id);
...
static platform_status rc_allocator_init_meta_page(rc_allocator *al) {
  ...
  al->meta_page = platform_aligned_malloc(al->heap_id, al->cfg->page_size,
                                          al->cfg->page_size);
...
void rc_allocator_config_init(rc_allocator_config *allocator_cfg,
...
platform_status rc_allocator_init(rc_allocator *al, rc_allocator_config *cfg,
                                  io_handle *io, platform_heap_handle hh,
...
platform_status rc_allocator_mount(rc_allocator *al, rc_allocator_config *cfg,
                                   io_handle *io, platform_heap_handle hh,
                                   platform_heap_id hid,
                                   platform_module_id mid) {
...
uint64 rc_allocator_get_capacity(rc_allocator *al) { return al->cfg->capacity; }
...
platform_status rc_allocator_alloc_super_addr(
    rc_allocator *al, allocator_root_id allocator_root_id, uint64 *addr) {
  platform_status status = STATUS_NOT_FOUND;
    ...
    if (al->meta_page->splinters[idx] == INVALID_ALLOCATOR_ROOT_ID) {
      ...
      *addr = (1 + idx) * al->cfg->page_size;
      al->meta_page->checksum =
          platform_checksum128(al->meta_page, sizeof(al->meta_page->splinters),
                               RC_ALLOCATOR_META_PAGE_CSUM_SEED);
      platform_status io_status = io_write(
          al->io, al->meta_page, al->cfg->page_size, RC_ALLOCATOR_BASE_OFFSET);
  ...
  for (uint8 idx = 0; idx < RC_ALLOCATOR_MAX_ALLOCATOR_ROOT_IDS; idx++) {
    ...
    if (al->meta_page->splinters[idx] == allocator_root_id) {
      ...
      al->meta_page->checksum =
          platform_checksum128(al->meta_page, sizeof(al->meta_page->splinters),
                               RC_ALLOCATOR_META_PAGE_CSUM_SEED);
      platform_status status = io_write(
          al->io, al->meta_page, al->cfg->page_size, RC_ALLOCATOR_BASE_OFFSET);
      platform_assert_status_ok(status);
      platform_mutex_unlock(&al->lock);
      return;
...
uint64 rc_allocator_page_size(rc_allocator *al) { return al->cfg->page_size; }
...
platform_status rc_allocator_alloc_extent(rc_allocator *al, /* IN  */
                                          uint64 *addr)     /* OUT */
{
