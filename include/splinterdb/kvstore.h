...
//
// Any thread that uses a kvstore must first be registered with it.
//
...
//
// kvstore_close will use scratch space, so the thread that calls it must
...
void
kvstore_register_thread(kvstore *kvs);
