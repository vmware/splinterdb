...
btree_config *test_memtable_context_btree_config(test_memtable_context *ctxt) {
  return ctxt->cfg->mt_cfg->btree_cfg;
}

void test_btree_process_noop(void *arg, uint64 generation) {
  // really a no-op
}

test_memtable_context *test_memtable_context_create(cache *cc,
                                                    test_btree_config *cfg,
                                                    uint64 num_mt,
...
void test_memtable_context_destroy(test_memtable_context *ctxt,
                                   platform_heap_id hid) {
  memtable_context_destroy(hid, ctxt->mt_ctxt);
}

platform_status test_btree_insert(test_memtable_context *ctxt, const char *key,
                                  const char *data) {
  uint64 generation;
...
bool test_btree_data_equal(btree_config *cfg, const char *data1,
                           const char *data2) {
  return memcmp(data1, data2, btree_message_size(cfg)) == 0;
}

bool test_btree_lookup(cache *cc, btree_config *cfg, uint64 root_addr,
                       const char *key, char *data, bool expected_found,
...
bool test_memtable_lookup(test_memtable_context *ctxt, uint64 mt_no,
                          const char *key, char *data, bool expected_found,
                          const char *expected_data) {
...
void test_btree_tuple(test_memtable_context *ctxt, char *key, char *data,
                      uint64 seq, uint64 thread_id) {
  ...
  if (data != NULL) {
    test_insert_data((data_handle *)data, 1, (char *)&seq, 8,
                     btree_message_size(cfg->mt_cfg->btree_cfg),
                     MESSAGE_TYPE_INSERT);
  }
}
...
void test_btree_insert_thread(void *arg) {
...
static platform_status test_btree_perf(cache *cc, test_btree_config *cfg,
                                       uint64 num_inserts, uint64 num_threads,
                                       uint64 num_trees, task_system *ts,
                                       platform_heap_id hid) {
...
static void btree_test_async_callback(btree_async_ctxt *btree_ctxt) {
...
static void btree_test_put_async_ctxt(btree_test_async_lookup *async_lookup,
                                      btree_test_async_ctxt *ctxt) {
...
static void btree_test_async_ctxt_init(btree_test_async_lookup *async_lookup) {
...
btree_test_async_ctxt_any_used(const btree_test_async_lookup *async_lookup) {
  debug_assert(
      (async_lookup->ctxt_bitmap & ~((1UL << max_async_inflight) - 1)) == 0);
  return async_lookup->ctxt_bitmap != (1UL << max_async_inflight) - 1;
}

static bool btree_test_run_pending(cache *cc, btree_config *cfg,
                                   uint64 root_addr,
  ...
  for (i = 0; i < max_async_inflight; i++) {
    ...
    ctxt->ready = FALSE;
    res = btree_lookup_async(cc, cfg, root_addr, ctxt->key, ctxt->data, &found,
                             &ctxt->ctxt);
    switch (res) {
    case async_locked:
    case async_no_reqs:
...
static void btree_test_wait_pending(cache *cc, btree_config *cfg,
                                    uint64 root_addr,
                                    btree_test_async_lookup *async_lookup,
                                    bool expected_found) {
...
cache_async_result
test_btree_async_lookup(cache *cc, btree_config *cfg,
                        btree_test_async_ctxt *async_ctxt,
                        btree_test_async_lookup *async_lookup, uint64 root_addr,
                        const char *key, bool expected_found, bool *correct) {
  bool found;
  cache_async_result res;
...
cache_async_result test_memtable_async_lookup(
    test_memtable_context *ctxt, btree_test_async_ctxt *async_ctxt,
    btree_test_async_lookup *async_lookup, uint64 mt_no, const char *key,
    bool expected_found, bool *correct) {
...
static platform_status test_btree_basic(cache *cc, test_btree_config *cfg,
                                        platform_heap_id hid,
                                        uint64 num_inserts) {
  ...
  for (uint64 insert_num = 0; insert_num < num_inserts; insert_num++) {
    btree_test_async_ctxt *async_ctxt = btree_test_get_async_ctxt(async_lookup);
    ...
    if (ctxt == NULL) {
      ...
      cache_async_result res = test_memtable_async_lookup(
          ctxt, async_ctxt, async_lookup, 0, key, TRUE, &correct);
    ...
    btree_test_run_pending(cc, mt->cfg, mt->root_addr, async_lookup, async_ctxt,
                           TRUE);
  }
  ...
  for (uint64 insert_num = 0; insert_num < num_inserts; insert_num++) {
    ...
    if (async_ctxt == NULL) {
      ...
      test_btree_tuple(ctxt, key, expected_data, insert_num, 0);
      cache_async_result res =
          test_btree_async_lookup(cc, btree_cfg, async_ctxt, async_lookup,
                                  packed_root_addr, key, TRUE, &correct);
      if (res == async_success) {
        if (!correct) {
          btree_print_tree(cc, btree_cfg, packed_root_addr);
          char key_string[128];
  ...
  for (uint64 insert_num = start_num; insert_num < end_num; insert_num++) {
    test_btree_tuple(ctxt, key, expected_data, insert_num, 0);
    ZERO_CONTENTS(data);
    bool correct = test_btree_lookup(cc, btree_cfg, packed_root_addr, key, data,
                                     FALSE, NULL);
    if (!correct) {
      btree_print_tree(cc, btree_cfg, packed_root_addr);
      char key_string[128];
...
uint64 test_btree_create_packed_trees(cache *cc, test_btree_config *cfg,
                                      platform_heap_id hid, uint64 num_trees,
                                      uint64 *root_addr) {
  test_memtable_context *ctxt =
      test_memtable_context_create(cc, cfg, num_trees, hid);
...
static platform_status test_btree_merge_basic(cache *cc, test_btree_config *cfg,
                                              platform_heap_id hid,
                                              uint64 arity) {
  ...
  for (uint64 pivot_no = 0; pivot_no < arity; pivot_no++) {
    ...
    for (uint64 tree_no = 0; tree_no < arity; tree_no++) {
      btree_iterator_init(cc, btree_cfg, &btree_itor_arr[tree_no],
                          root_addr[tree_no], PAGE_TYPE_BRANCH,
    ...
    rc = merge_iterator_create(hid, btree_cfg->data_cfg, NULL, arity, itor_arr,
                               TRUE, TRUE, TRUE, &merge_itor);
    if (!SUCCESS(rc)) {
      goto destroy_btrees;
    }
    ...
    rc = test_count_tuples_in_range(cc, btree_cfg, root_addr, PAGE_TYPE_BRANCH,
    ...
    rc = test_count_tuples_in_range(cc, btree_cfg, &req.root_addr,
                                    PAGE_TYPE_BRANCH, 1, pivot_key[pivot_no].k,
                                    max_key, &output_count);
    if (!SUCCESS(rc)) {
      merge_iterator_destroy(hid, &merge_itor);
      goto destroy_btrees;
    }
    ...
    if (input_count != output_count) {
      test_btree_print_all_keys(cc, btree_cfg, root_addr, PAGE_TYPE_BRANCH,
                                arity, pivot_key[pivot_no].k, max_key);
      platform_log("****\n");
      test_btree_print_all_keys(cc, btree_cfg, &req.root_addr, PAGE_TYPE_BRANCH,
                                1, pivot_key[pivot_no].k, max_key);
      platform_log(
...
static platform_status test_btree_count_in_range(cache *cc,
                                                 test_btree_config *cfg,
                                                 platform_heap_id hid,
                                                 uint64 iterations) {
  ...
  for (uint64 i = 0; i < iterations; i++) {
    ...
    uint64 count =
        btree_count_in_range(cc, btree_cfg, root_addr, min_key, max_key);
    ...
    rc = test_count_tuples_in_range(cc, btree_cfg, &root_addr, PAGE_TYPE_BRANCH,
                                    1, min_key, max_key, &iterator_count);
    platform_assert_status_ok(rc);
...
static platform_status test_btree_rough_iterator(cache *cc,
                                                 test_btree_config *cfg,
                                                 platform_heap_id hid,
                                                 uint64 num_trees) {
  ...
  for (uint64 tree_no = 0; tree_no < num_trees; tree_no++) {
    btree_iterator_init(cc, btree_cfg, &rough_btree_itor[tree_no],
                        root_addr[tree_no], PAGE_TYPE_BRANCH, min_key, max_key,
                        TRUE, FALSE, 1, data_type_point);
...
static platform_status test_btree_merge_perf(cache *cc, test_btree_config *cfg,
                                             platform_heap_id hid, uint64 arity,
                                             uint64 num_merges) {
...
int btree_test(int argc, char *argv[]) {
  io_config io_cfg;
  ...
  rc = test_parse_args(cfg, data_cfg, &io_cfg, &al_cfg, &cache_cfg, &log_cfg,
                       &seed, config_argc, config_argv);
