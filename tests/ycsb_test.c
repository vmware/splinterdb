 ...
 */
void sum_latency_tables(latency_table dest, latency_table table1,
...
uint64_t total_latency(latency_table table) {
  ...
  for (exponent = 0; exponent < LATENCY_EXPONENT_LIMIT; exponent++)
    for (mantissa = 0; mantissa < LATENCY_MANTISSA_LIMIT; mantissa++)
      total += table[exponent][mantissa] * compute_latency(exponent, mantissa);
  return total;
}
...
void print_latency_table(platform_stream_handle stream, latency_table table) {
  uint64_t exponent;
  uint64_t mantissa;
  bool started = 0;
...
static void ycsb_thread(void *arg) {
  platform_status rc;
  ...
  __sync_val_compare_and_swap(&params->times.earliest_thread_start_time, 0,
                              start_time);
  ...
  while (my_batch < num_ops) {
    ...
    for (i = 0; i < batch_size; i++) {
      ...
      switch (ops->cmd) {
      case 'r': {
        rc = splinter_lookup(spl, ops->key, ops->value, &ops->found);
      ...
      case 's': {
        ...
        rc = splinter_range(spl, ops->key, ops->range_len, &tuples_returned,
                            range_buffer);
        platform_assert_status_ok(rc);
        break;
      }
...
static int run_ycsb_phase(splinter_handle *spl, ycsb_phase *phase,
  ...
  for (i = 0; i < phase->nlogs; i++) {
    ...
    for (j = 0; j < phase->params[i].nthreads; j++) {
      ...
      ret = task_thread_create("ycsb_thread", ycsb_thread, &phase->params[i],
                               splinter_get_scratch_size(), ts, hid,
                               &threads[cur_thread]);
      if (!SUCCESS(ret)) {
        success = -1;
        goto shutdown;
  ...
  if (phase->measurement_command) {
    ...
    do {
      ...
      if (num_written != num_read) {
        platform_error_log("Could not write to measurement output file %s\n",
                           filename);
...
static int run_all_ycsb_phases(splinter_handle *spl, ycsb_phase *phase,
                               uint64 nphases, task_system *ts,
...
static void parse_ycsb_log_file(void *arg) {
...
static platform_status load_ycsb_logs(int argc, char *argv[], uint64 *nphases,
                                      bool *use_existing, ycsb_phase **output,
                                      int *args_consumed,
  ...
  *memory_bytes_out = MiB_TO_B(strtoull(argv[5], NULL, 0));
  ...
  for (lognum = 0; lognum < num_threads; lognum++) {
    ...
    ret = platform_thread_create(&params[lognum].thread, FALSE,
                                 parse_ycsb_log_file, req, hid);
...
void compute_log_latency_tables(ycsb_log_params *params) {
  ...
  sum_latency_tables(params->tables.all_queries, params->tables.pos_queries,
...
void write_log_latency_table(char *phase_name, uint64_t lognum,
                             char *operation_name, latency_table table) {
  char filename[1024];
  snprintf(filename, sizeof(filename), "%s.%02lu.%s.latency.df", phase_name,
           lognum, operation_name);
  write_latency_table(filename, table);
}

void write_log_latency_cdf(char *phase_name, uint64_t lognum,
                           char *operation_name, latency_table table) {
  char filename[1024];
  snprintf(filename, sizeof(filename), "%s.%02lu.%s.latency.cdf", phase_name,
           lognum, operation_name);
  write_latency_cdf(filename, table);
}

void write_phase_latency_table(char *phase_name, char *operation_name,
                               latency_table table) {
  char filename[1024];
  snprintf(filename, sizeof(filename), "%s.%s.latency.df", phase_name,
           operation_name);
  write_latency_table(filename, table);
}

void write_phase_latency_cdf(char *phase_name, char *operation_name,
                             latency_table table) {
  char filename[1024];
  snprintf(filename, sizeof(filename), "%s.%s.latency.cdf", phase_name,
           operation_name);
  write_latency_cdf(filename, table);
}
...
void print_operation_statistics(platform_log_handle output,
                                char *operation_name, latency_table table) {
...
void print_statistics_file(platform_log_handle output, uint64_t total_ops,
                           running_times *times, latency_tables *tables) {
  ...
  platform_handle_log(
      output, "mean_overall_latency: %f\n",
      total_ops ? 1.0 * times->sum_of_wall_clock_times / total_ops : 0);
  platform_handle_log(
      output, "mean_overall_throughput: %f\n",
...
void write_log_statistics_file(char *phase_name, uint64_t lognum,
                               ycsb_log_params *params) {
  char filename[1024];
  snprintf(filename, sizeof(filename), "%s.%02lu.statistics", phase_name,
           lognum);
  FILE *output = fopen(filename, "w");
  platform_assert(output != NULL);

  print_statistics_file(output, params->total_ops, &params->times,
                        &params->tables);
...
void write_phase_statistics_files(ycsb_phase *phase) {
  ...
  print_statistics_file(output, phase->total_ops, &phase->times,
                        &phase->tables);
...
int ycsb_test(int argc, char *argv[]) {
  io_config io_cfg;
  ...
  rc = load_ycsb_logs(argc, argv, &nphases, &use_existing, &phases,
                      &args_consumed, &log_size_bytes, &memory_bytes);
  ...
  uint64 overhead_bytes =
      memory_bytes / splinter_cfg->page_size * sizeof(uint32) +
  ...
  snprintf(resize_hugetlb_command, 1024, "echo %lu > /proc/sys/vm/nr_hugepages",
           huge_tlb_pages);
  ...
  if (use_existing) {
    ...
    platform_assert_status_ok(rc);
    spl = splinter_mount(splinter_cfg, (allocator *)&al, (cache *)cc, ts,
    ...
    rc = clockcache_init(cc, &cache_cfg, (io_handle *)io, (allocator *)&al,
                         "test", ts, hh, hid, platform_get_module_id());
    platform_assert_status_ok(rc);
    spl = splinter_create(splinter_cfg, (allocator *)&al, (cache *)cc, ts,
                          test_generate_allocator_root_id(), hid);
    platform_assert(spl);
  }
