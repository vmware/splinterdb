...
platform_status test_cache_page_pin(cache *cc, page_handle **page_arr,
...
static platform_status cache_test_alloc_extents(cache *cc,
  ...
  for (ea = 0; ea < extents_to_allocate; ea++) {
    ...
    for (uint32 i = 0; i < cfg->pages_per_extent; i++) {
      addr_arr[ea * cfg->pages_per_extent + i] = extent_page_arr[i]->disk_addr;
...
platform_status test_cache_basic(cache *cc, clockcache_config *cfg,
  ...
  if (dirty_count != 0) {
    platform_error_log("Expected no dirty entries but found: %u", dirty_count);
    rc = STATUS_TEST_FAILED;
    goto exit;
  }
...
static void cache_test_index_itor_mono_init(cache_test_index_itor *itor,
                                            uint32 cur, int32 incr) {
  itor->type = MONO;
  itor->mono.cur = cur;
  itor->mono.incr = incr;
}

static void cache_test_index_itor_rand_init(cache_test_index_itor *itor,
                                            uint32 seed, uint32 min,
                                            uint32 max) {
  itor->type = RAND;
...
static void cache_test_index_itor_hop_init(cache_test_index_itor *itor,
...
static uint32 cache_test_index_itor_get(cache_test_index_itor *itor) {
  uint32 idx;
...
static platform_status cache_test_dirty_flush(cache *cc, clockcache_config *cfg,
                                              const char *testname,
...
platform_status test_cache_flush(cache *cc, clockcache_config *cfg,
                                 platform_heap_id hid,
  ...
  cache_test_index_itor_rand_init(&itor, 42, cfg->page_capacity * 4,
...
void test_async_callback(cache_async_ctxt *ctxt) {
...
static void test_wait_inflight(test_params *params,
...
static void test_abandon_read_batch(test_params *params, uint64 batch_start,
                                    uint64 batch_end, // exclusive
                                    bool was_async[]) {
...
static bool test_do_read_batch(threadid tid, test_params *params,
                               uint64 batch_start) {
 ...
 * eviction.
 */
platform_status test_cache_async(cache *cc, clockcache_config *cfg,
                                 platform_heap_id hid, task_system *ts,
  ...
  for (i = 0; i < total_threads; i++) {
    ...
    if (is_reader) {
      rc = task_thread_create("cache_reader", test_reader_thread, &params[i], 0,
                              ts, hid, &params[i].thread);
    } else {
      rc = task_thread_create("cache_writer", test_writer_thread, &params[i], 0,
                              ts, hid, &params[i].thread);
    }
    if (!SUCCESS(rc)) {
      total_threads = i;
