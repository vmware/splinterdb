...
static inline void test_data_set_insert_flag(void *raw_data) {
  data_handle *data = (data_handle *)raw_data;
  data->message_type = MESSAGE_TYPE_INSERT;
}

static inline void test_data_set_delete_flag(void *raw_data) {
  data_handle *data = (data_handle *)raw_data;
  data->message_type = MESSAGE_TYPE_DELETE;
}

static inline void test_data_set_insert(void *raw_data, int8 ref_count,
                                        char *val, uint64 data_size) {
...
static inline void test_data_print_key(const void *key) {
