...
int cache_test(int argc, char *argv[]);
...
test_init_splinter(platform_heap_id hid, platform_io_handle *ioh,
...
static inline void test_deinit_splinter(platform_heap_id hid, task_system *ts) {
  task_system_destroy(hid, ts);
}

static inline void test_key(char *key, test_key_type key_type, uint64 idx,
                            uint64 thread_id, uint64 semiseq_freq,
                            uint64 key_size, uint64 period) {
  ...
  case TEST_SEQ:
    *(uint64 *)key =
        htobe64(platform_checksum64(&thread_id, sizeof(thread_id), 42) + idx);
    break;
  case TEST_SEMISEQ:
...
static inline bool test_period_complete(uint64 idx, uint64 period) {
  return idx % period == 0;
}

static inline uint64 test_range(uint64 idx, uint64 range_min,
                                uint64 range_max) {
  debug_assert(range_max > range_min);
  return range_min +
         platform_checksum64(&idx, sizeof(idx), 43) % (range_max - range_min);
}

static inline void test_int_to_key(char *key, uint64 idx, uint64 key_size) {
  memset(key, 0, key_size);
  *(uint64 *)key = htobe64(idx);
}

static inline void test_insert_data(void *raw_data, int8 ref_count, char *val,
                                    uint64 input_size, uint64 data_size,
                                    message_type type) {
...
static inline platform_status
test_count_tuples_in_range(cache *cc, btree_config *cfg, uint64 *root_addr,
                           page_type type, uint64 num_trees, char *low_key,
                           char *high_key,
                           uint64 *count) // OUTPUT
{
  btree_iterator itor;
  ...
  *count = 0;
  for (i = 0; i < num_trees; i++) {
    ...
    btree_iterator_init(cc, cfg, &itor, root_addr[i], type, low_key, high_key,
                        TRUE, FALSE, 0, data_type_point);
    bool at_end;
...
static inline int test_btree_print_all_keys(cache *cc, btree_config *cfg,
                                            uint64 *root_addr, page_type type,
                                            uint64 num_trees, char *low_key,
                                            char *high_key) {
  btree_iterator itor;
  uint64 i;
  for (i = 0; i < num_trees; i++) {
...
static inline void
test_config_init(splinter_config *splinter_cfg, data_config *data_cfg,
                 shard_log_config *log_cfg, clockcache_config *cache_cfg,
                 rc_allocator_config *allocator_cfg, io_config *io_cfg,
  ...
  *data_cfg = *test_data_config;
  ...
  splinter_config_init(
      splinter_cfg, data_cfg, (log_config *)log_cfg,
      master_cfg->memtable_capacity, master_cfg->fanout,
      master_cfg->max_branches_per_node, master_cfg->btree_rough_count_height,
      master_cfg->page_size, master_cfg->extent_size,
      master_cfg->filter_remainder_size, master_cfg->filter_index_size,
      master_cfg->reclaim_threshold, master_cfg->use_log,
      master_cfg->use_stats);
}

static inline platform_status
test_parse_args_n(splinter_config *splinter_cfg, data_config *data_cfg,
                  io_config *io_cfg, rc_allocator_config *allocator_cfg,
                  clockcache_config *cache_cfg, shard_log_config *log_cfg,
                  uint64 *seed, uint8 num_config, int argc, char *argv[]) {
  platform_status rc;
  uint8 i;
...
static inline platform_status
test_parse_args(splinter_config *splinter_cfg, data_config *data_cfg,
                io_config *io_cfg, rc_allocator_config *allocator_cfg,
                clockcache_config *cache_cfg, shard_log_config *log_cfg,
                uint64 *seed, int argc, char *argv[]) {
...
static inline allocator_root_id test_generate_allocator_root_id() {
