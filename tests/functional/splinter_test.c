...
/*
 * -----------------------------------------------------------------------------
 * splinter_setup_perf_inserts()
 *
 * Setup tables and thread-parameters in preparation for insert performance
 * workload. Computes the per-table insert workloads, and returns total_inserts.
 * -----------------------------------------------------------------------------
 */
static void
splinter_setup_perf_inserts(trunk_handle               **spl_tables,
                            trunk_config                *cfg,
                            test_config                 *test_cfg,
                            task_system                 *ts,
                            test_splinter_thread_params *params,
 ...
 *
 * Run the insert performance workload with n-threads.
 */
static platform_status
...
/*
 * -----------------------------------------------------------------------------
 * test_splinter_periodic() --
 *
 * SplinterDB Periodic Performance exerciser. For a specified # of threads of
 * each type, execute:
 *  - Inserts
 *  - Lookups
 *  - Range queries, of 3 diff ranges { small, medium, large }
 *
 * Similar to test_splinter_perf()
 * -----------------------------------------------------------------------------
 */
platform_status
