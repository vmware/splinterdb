...
test_trunk_insert_thread(void *arg)
{
   ...
   uint64 insert_base = 0;
   while (1) {
      test_print_progress(&params->progress,
                          insert_base / (total_ops / 100),
                          PLATFORM_CR "Thread %lu inserting %3lu%% complete",
                          thread_number,
                          insert_base / (total_ops / 100));
      insert_base = __sync_fetch_and_add(curr_op, op_granularity);
      if (insert_base >= total_ops) {
         platform_default_log(" Test done.\n");
         goto out;
      }
      ...
      for (uint64 op_offset = 0; op_offset != op_granularity; op_offset++) {
         uint64 insert_num = insert_base + op_offset;
...
test_trunk_lookup_thread(void *arg)
{
   ...
   test_async_lookup *async_lookup   = params->async_lookup;
   ...
   uint64 lookup_base = 0;
   while (1) {
      test_print_progress(&params->progress,
                          lookup_base / (total_ops / 100),
                          PLATFORM_CR "Thread %lu lookups %3lu%% complete",
                          thread_number,
                          lookup_base / (total_ops / 100));
      lookup_base = __sync_fetch_and_add(curr_op, op_granularity);
      if (lookup_base >= total_ops) {
         goto out;
      }
      ...
      for (uint64 op_offset = 0; op_offset != op_granularity; op_offset++) {
         ...
         uint64           lookup_num = lookup_base + op_offset;
         timestamp        ts;
         ...
         if (async_lookup->max_async_inflight == 0) {
            ...
            verify_tuple(
               ...
               lookup_num,
               key_buffer_key(&keybuf),
               merge_accumulator_to_message(lookup_result_accumulator(&data)),
               expected_found);
         } else {
...
test_trunk_range_thread(void *arg)
{
   ...
   uint64 range_base = 0;
   while (1) {
      uint64 pct_done = (range_base / (total_ops / 100));
      char  *newmsg   = "";
      ...
      test_print_progress(&params->progress,
                          pct_done,
                          PLATFORM_CR "Thread %lu range lookups %3lu%% "
                                      "complete, range_base=%lu%s",
                          thread_number,
                          pct_done,
                          range_base,
                          newmsg);

      range_base = __sync_fetch_and_add(curr_op, op_granularity);
      if (range_base >= total_ops) {
         goto out;
      }

      for (uint64 op_offset = 0; op_offset != op_granularity; op_offset++) {
         uint64 range_num = range_base + op_offset;
         test_key(&start_key,
                  test_cfg->key_type,
                  range_num,
                  thread_number,
             ...
             test_splinter_pthread_op_type type)
{
   ...
   if (type == OP_INSERT) {
      test_print_progress(&params->progress,
                          ...
                          *base / (total_ops / 100));
      *base = __sync_fetch_and_add(curr_op, op_granularity);
      if (*base >= total_ops) {
         return TRUE;
      }
   } else {
      ...
      if (random_next_uint64(rs) % 100 < lookup_positive_pct) {
         if (local_curr_op == op_granularity) {
            ...
            random_base =
               ((random_next_uint64(rs) % (local_curr_op / op_granularity - 1))
                + 1)
               * op_granularity;
         }
      } else {
         if (local_curr_op >= total_ops - op_granularity) {
            // one interval left,
...
test_trunk_insert_lookup_thread(void *arg)
{
   ...
   for (uint8 i = 0; i < NUM_OP_TYPES; i++) {
      advance_base(params, params->curr_op, &bases[i], &rs, i);
   }
   ...
   while (1) {
      for (uint8 op_type = 0; op_type != NUM_OP_TYPES; op_type++) {
             ...
             || offsets[op_type] >= op_granularity)
         {
            ...
            if (advance_base(
                   params, params->curr_op, &bases[op_type], &rs, op_type))
            {
               goto out;
            }
            offsets[op_type] = 0;
...
out:
   test_wait_for_inflight(spl, params->async_lookup, &vtarg);
...
/*
 * The unified superblock is per-device: core owns one superblock per device, so
 * these tests drive exactly one table.
 */
static platform_status
test_trunk_create_table(core_handle    **spl_handle,
                        system_config   *cfg,
                        allocator       *al,
                        cache           *cc,
                        task_system     *ts,
...
static void
test_trunk_destroy_table(core_handle *spl, platform_heap_id hid)
{
   core_destroy(spl);
   platform_free(hid, spl);
}
 ...
 * compute_num_inserts() --
 *
 * Helper function to compute the # of inserts for the table, based on the
 * configuration specified.
 */
static uint64
compute_num_inserts(test_config *test_cfg) // IN
{
   uint64 num_inserts_cfg = test_cfg->test_exec_cfg->num_inserts;
   uint64 tuple_size =
      test_cfg->key_size + generator_average_message_size(test_cfg->gen);
   uint64 num_inserts =
      (num_inserts_cfg ? num_inserts_cfg : test_cfg->tree_size / tuple_size);

   if (test_cfg->key_type == TEST_PERIODIC) {
      test_cfg->period = num_inserts;
      num_inserts *= test_cfg->num_periods;
   }
   return ROUNDUP(num_inserts, TEST_INSERT_GRANULARITY);
}
                   ...
                   bool32                       is_parallel)
{
   for (uint64 i = 0; i < num_threads; i++) {
      ...
      params[i].total_ops      = num_inserts;
 ...
 *
 * Helper function to setup Async contexts for num_threads threads. Lookup
 * execution needs to access this context before it can check if
...
{
   for (uint64 i = 0; i < num_threads; i++) {
      async_ctxt_init(hid, max_async_inflight, &params[i].async_lookup);
   }
}
 ...
 *
 * Helper function to dismantle n-Async contexts.
 */
static void
...
{
   for (uint64 i = 0; i < num_threads; i++) {
      async_ctxt_deinit(hid, params[i].async_lookup);
      params[i].async_lookup = NULL;
   }
}
...
{
   platform_status rc;
   *total_inserts = compute_num_inserts(test_cfg);
   ...
   cache_io_stats(cc, &read_io_bytes, &write_io_bytes);
                      ...
                      uint64                       total_inserts)
{
   ...
   do_n_async_ctxt_inits(hid, num_lookup_threads, max_async_inflight, params);
   ...
   do_n_async_ctxt_deinits(hid, num_lookup_threads, params);
                            ...
                            trunk_range_perf_params     *range_perf)
{
   ...
   bool32 verbose_progress =
      test_show_verbose_progress(test_cfg->test_exec_cfg);
   uint64 num_ranges_total =
      ROUNDUP(num_inserts / num_ranges, TEST_RANGE_GRANULARITY);
   if (verbose_progress) {
      platform_default_log(
         " num_inserts=%lu, num_ranges=%lu\n", num_inserts, num_ranges_total);
   }

   for (uint64 i = 0; i < num_range_threads; i++) {
      params[i].total_ops             = num_ranges_total;
      ...
      if (verbose_progress) {
         platform_default_log(" Range thread[%lu] "
                              "total_ops=%lu, op_granularity=%lu"
                   ...
                   uint64           insert_rate)
{
   ...
   rc = test_trunk_create_table(&spl, cfg, al, cc, ts, hid);
   if (!SUCCESS(rc)) {
...
destroy_splinter:
   test_trunk_destroy_table(spl, hid);
...
{
   platform_default_log(
      "splinter_test: SplinterDB performance test (periodic) started\n");
   core_handle    *spl;
   platform_status rc;
   ...
   uint64 tuple_size =
      test_cfg->key_size + generator_average_message_size(test_cfg->gen);
   uint64 num_inserts = test_cfg->tree_size / tuple_size;
   if (test_cfg->key_type == TEST_PERIODIC) {
      test_cfg->period = num_inserts;
      num_inserts *= test_cfg->num_periods;
   }
   uint64 total_inserts = ROUNDUP(num_inserts, TEST_INSERT_GRANULARITY);
   ...
   for (uint64 i = 0; i < num_threads; i++) {
      params[i].spl            = spl;
      params[i].test_cfg       = test_cfg;
      params[i].total_ops      = total_inserts;
      params[i].curr_op        = &curr_op;
      params[i].op_granularity = TEST_INSERT_GRANULARITY;
      params[i].thread_number  = i;
      params[i].expected_found = TRUE;
   ...
   uint64 total_ranges = ROUNDUP(total_inserts / 128, TEST_RANGE_GRANULARITY);

   for (uint64 i = 0; i < num_threads; i++) {
      params[i].total_ops        = total_ranges;
      params[i].op_granularity   = TEST_RANGE_GRANULARITY;
   ...
   if (num_range_threads != 0) {
      ...
      curr_op      = 0;
      total_ranges = ROUNDUP(total_ranges / 4, TEST_RANGE_GRANULARITY);
      for (uint64 i = 0; i < num_range_threads; i++) {
         params[i].total_ops        = total_ranges;
         params[i].op_granularity   = TEST_RANGE_GRANULARITY;
                            ...
                            platform_heap_id hid)
{
   ...
   platform_default_log(
      "splinter_test: SplinterDB parallel performance test started\n");
   core_handle    *spl;
   platform_status rc;
   ...
   uint64 total_inserts = compute_num_inserts(test_cfg);
   ...
   do_n_async_ctxt_inits(hid, num_threads, max_async_inflight, params);
   ...
   do_n_async_ctxt_deinits(hid, num_threads, params);
...
test_splinter_delete(system_config   *cfg,
                     ...
                     uint64           insert_rate)
{
   platform_default_log("splinter_test: SplinterDB deletion test started\n");
   core_handle    *spl;
   platform_status rc;
...
splinter_test(int argc, char *argv[])
{
   ...
   test_config *test_cfg = TYPED_ZALLOC(hid, test_cfg);
   test_config_set_defaults(test, test_cfg);
   test_cfg->gen           = &gen;
   test_cfg->test_exec_cfg = &test_exec_cfg;

   rc = test_config_parse(test_cfg, 1, &config_argc, &config_argv);
   if (!SUCCESS(rc)) {
    ...
    */
   system_config        *system_cfg   = TYPED_ZALLOC(hid, system_cfg);
   test_workload_config *workload_cfg = TYPED_ZALLOC(hid, workload_cfg);
   ...
   test_cfg->key_size = workload_cfg->key_size;
   ...
   for (task_type type = 0; type != NUM_TASK_TYPES; type++) {
      total_threads += system_cfg->task_cfg.num_background_threads[type];
   }
   // Check if IO subsystem has enough reqs for max async IOs inflight
   if (system_cfg->io_cfg.kernel_queue_size
       < total_threads * max_async_inflight)
   {
      system_cfg->io_cfg.kernel_queue_size =
   ...
   clockcache *cc = TYPED_ZALLOC(hid, cc);
   ...
   switch (test) {
      ...
      case parallel_perf:
         platform_assert(
            max_async_inflight == 0
            || (0 < system_cfg->task_cfg
                       .num_background_threads[TASK_TYPE_MEMTABLE]
                && 0 < system_cfg->task_cfg
                          .num_background_threads[TASK_TYPE_NORMAL]));
      ...
      case periodic:
         ...
         break;
      case functionality:
         system_cfg->splinter_cfg.data_cfg->key_to_string =
