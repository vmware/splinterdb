...
splinter_io_apis_test(int argc, char *argv[])
{
   ...
   // Run this block if we are in the main thread (i.e. didn't fork) or if we
   // are a child process created after fork()'ing.
   if (!master_cfg.fork_child || (pid == 0)) {
      ...
      if (pid == 0) { // In child process ...
         ...
         io_test_fn_arg.arg_io_hdlp   = io_hdl;
         io_test_fn_arg.arg_io_ctxt_t = io_ctxt;
       ...
       *
       * Currently we cannot support forked children themselves starting up
       * threads and accessing Splinter.
       */
      if (pid == 0) {
         // The forked child process which uses Splinter as a "thread" needs to
         // relinquish its resources before exiting.
                 ...
                 char                stamp_char)
{
   ...
   for (uint64 curr = start_addr; curr < end_addr; curr += page_size, num_IOs++)
   {
      ...
      if (!SUCCESS(rc)) {
         platform_error_log("\n**** Error! Write IO at addr %lu wrote %d bytes"
...
test_sync_writes_worker(void *arg)
{
   ...
   platform_assert((argp->arg_io_ctxt_t != act_ctxt),
                   "In %s process, tid=%lu: Actual opaque IO context handle, %p"
                ...
                char                stamp_char)
{
   ...
   for (uint64 curr = start_addr; curr < end_addr; curr += page_size, num_IOs++)
   {
      ...
      if (!SUCCESS(rc)) {
         platform_error_log("\n**** Error! Read IO at addr %lu read %d bytes"
                   ...
                   int              nthreads)
{
   ...
   for (int i = 0; i < nthreads; i++, param++) {
      ...
      param->arg_whoami     = io_test_param->arg_whoami;
   }
}
