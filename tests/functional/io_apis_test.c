...
splinter_io_apis_test(int argc, char *argv[])
{
   ...
   if (!master_cfg.fork_child || (pid == 0)) {
      ...
      rc = test_sync_write_reads_by_threads(&io_test_fn_arg, NUM_THREADS);
      platform_assert_status_ok(rc);
...
test_sync_writes_worker(void *arg)
{
   ...
   // For shmem: each thread should have its own IO context
...
test_async_reads_worker(void *arg)
{
   ...
   platform_assert((argp->io_ctxt != act_ctxt),
                   "Actual opaque IO context handle, %p"
                   " should not match the shared context handle, %p\n",
                   act_ctxt,
                   argp->io_ctxt);
