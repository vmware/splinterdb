...
int test_log_basic(cache *cc, shard_log_config *cfg, shard_log *log,
                   uint64 num_entries, platform_heap_id hid) {
  platform_status rc;
  log_handle *logh;
  uint64 i;
  char key[MAX_KEY_SIZE];
  char *data = TYPED_ARRAY_MALLOC(hid, data, cfg->data_cfg->message_size);
  char *returned_key;
  char *returned_data;
  char dummy = 'z';
  uint64 addr;
  ...
  for (i = 0; i < num_entries; i++) {
    ...
    test_insert_data(data, 1, &dummy, 0, cfg->data_cfg->message_size,
                     MESSAGE_TYPE_INSERT);
...
int test_log_crash(clockcache *cc, clockcache_config *cache_cfg, io_handle *io,
                   allocator *al, shard_log_config *cfg, shard_log *log,
                   uint64 num_entries, task_system *ts, platform_heap_handle hh,
...
void test_log_thread(void *arg) {
  ...
  for (i = thread_id * num_entries; i < (thread_id + 1) * num_entries; i++) {
    ...
    test_insert_data(data, 1, &dummy, 0, log->cfg->data_cfg->message_size,
                     MESSAGE_TYPE_INSERT);
    log_write(logh, key, data, i);
  }
...
platform_status test_log_perf(cache *cc, shard_log_config *cfg, shard_log *log,
                              uint64 num_entries, uint64 num_threads,
                              task_system *ts, platform_heap_id hid)

{
  test_log_thread_params *params = TYPED_ARRAY_MALLOC(hid, params, num_threads);
  ...
  for (uint64 i = 0; i < num_threads; i++) {
    ret = task_thread_create("log_thread", test_log_thread, &params[i], 0, ts,
                             hid, &params[i].thread);
    if (!SUCCESS(ret)) {
