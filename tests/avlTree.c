...
static inline void AvlTreeSetNode(AvlTreeLinks *n, AvlTreeLinks *left,
                                  AvlTreeLinks *right, uint32 height) {
  n->left = left;
  n->right = right;
  n->height = height;
}

static inline uint32 AvlTreeGetHeight(AvlTreeLinks *n) {
  if (UNLIKELY(n == NULL)) {
    return 0;
  }
...
void AvlTree_Init(AvlTree *tree,                 // IN/OUT
...
static void AvlTreeUpdateHeight(const AvlTree *tree, // IN
...
static AvlTreeLinks *AvlTreeRotateLeft(const AvlTree *tree, // IN
...
static AvlTreeLinks *AvlTreeRotateRight(const AvlTree *tree, // IN
                                        AvlTreeLinks *n2,    // IN/OUT
                                        AvlTreeLinks *n1)    // IN/OUT
{
  n2->right = n1->left;
...
static AvlTreeLinks *AvlTreeRebalanceNode(const AvlTree *tree, // IN/OUT
                                          AvlTreeLinks *n,     // IN/OUT
...
static AvlTreeLinks *AvlTreeInsertInt(const AvlTree *tree,   // IN
...
void AvlTree_Insert(AvlTree *tree,      // IN/OUT
...
static AvlTreeLinks *AvlTreeDeleteInt(const AvlTree *tree,     // IN
...
void AvlTree_Delete(AvlTree *tree,      // IN/OUT
                    AvlTreeLinks *node) // IN/OUT
{
  AvlTreeLinks *root = tree->root;
...
AvlTreeLinks *AvlTree_FindNode(const AvlTree *tree, // IN
                               AvlTreeKey key)      // IN
{
...
AvlTreeLinks *AvlTree_FindNodeGeq(const AvlTree *tree, // IN
                                  AvlTreeKey key)      // IN
{
...
AvlTreeLinks *AvlTree_FindNodeLeq(const AvlTree *tree, // IN
                                  AvlTreeKey key)      // IN
{
  AvlTreeLinks *res = NULL;
  AvlTreeLinks *current = tree->root;
...
AvlTreeLinks *AvlTree_Min(const AvlTree *tree) // IN
{
  AvlTreeLinks *res = NULL;
  AvlTreeLinks *n = tree->root;
  while (n != NULL) {
...
AvlTreeLinks *AvlTree_Max(const AvlTree *tree) // IN
{
  AvlTreeLinks *res = NULL;
  AvlTreeLinks *n = tree->root;
  while (n != NULL) {
...
AvlTreeLinks *AvlTree_Successor(const AvlTree *tree,      // IN
...
AvlTreeLinks *AvlTree_Predecessor(const AvlTree *tree,      // IN
                                  const AvlTreeLinks *node) // IN
{
  AvlTreeLinks *res = NULL;
  AvlTreeLinks *current = tree->root;
...
static void AvlTreeStackInit(AvlTreeIter *iter, unsigned max) {
  iter->num = 0;
  iter->max = max;
}
...
static void AvlTreeStackPush(AvlTreeIter *iter, AvlTreeLinks *cur) {
...
static AvlTreeLinks *AvlTreeStackPop(AvlTreeIter *iter) {
...
static bool AvlTreeStackIsEmpty(AvlTreeIter *iter) { return iter->num == 0; }
...
void AvlTreeIter_Init(AvlTreeIter *iter, // IN iterator
...
bool AvlTreeIter_IsAtEnd(AvlTreeIter *iter) { return iter->cur == NULL; }
