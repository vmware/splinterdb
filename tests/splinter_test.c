...
static inline bool test_is_done(const uint8 done, const uint8 n) {
  return ((done >> n) & 1);
}

static inline void test_set_done(uint8 *done, const uint8 n) {
  *done |= 1 << n;
}

static inline bool test_all_done(const uint8 done, const uint8 num_tables) {
  return (done == ((1 << num_tables) - 1));
}

void test_splinter_insert_thread(void *arg) {
  ...
  while (1) {
    ...
    for (uint64 op_offset = 0; op_offset != op_granularity; op_offset++) {
      for (uint8 spl_idx = 0; spl_idx < num_tables; spl_idx++) {
        ...
        test_key(key, test_cfg[spl_idx].key_type, insert_num, thread_number,
                 test_cfg[spl_idx].semiseq_freq, splinter_key_size(spl),
                 test_cfg[spl_idx].period);
        test_insert_data((data_handle *)data, 1, (char *)&insert_num, 8,
                         splinter_message_size(spl), type);
...
static void verify_tuple(splinter_handle *spl, uint64 lookup_num, char *key,
                         const char *data, char *expected_data,
                         size_t data_size, bool expected_found, bool found) {
...
static void verify_tuple_callback(splinter_handle *spl, test_async_ctxt *ctxt,
                                  bool found, void *arg) {
  ...
  verify_tuple(spl, ctxt->lookup_num, ctxt->key, ctxt->data, vta->expected_data,
               vta->data_size, vta->expected_found, found);
}

static void test_wait_for_inflight(splinter_handle *spl,
...
static test_async_ctxt *test_async_ctxt_get(splinter_handle *spl,
                                            test_async_lookup *async_lookup,
                                            verify_tuple_arg *vtarg) {
...
void test_splinter_lookup_thread(void *arg) {
  test_splinter_thread_params *params = (test_splinter_thread_params *)arg;
    ...
    for (uint64 op_offset = 0; op_offset != op_granularity; op_offset++) {
      ...
      for (spl_idx = 0; spl_idx < num_tables; spl_idx++) {
        ...
        if (async_lookup->max_async_inflight == 0) {
          ...
          test_key(key, test_cfg[spl_idx].key_type, lookup_num, thread_number,
                   test_cfg[spl_idx].semiseq_freq, splinter_key_size(spl),
                   test_cfg[spl_idx].period);
          ts = platform_get_timestamp();
...
void test_splinter_range_thread(void *arg) {
  test_splinter_thread_params *params = (test_splinter_thread_params *)arg;
...
static bool advance_base(const test_splinter_thread_params *params,
                         uint64 *curr_op, uint64 *base, uint8 *done, PRG *rs,
  ...
  for (uint8 spl_idx = 0; spl_idx < num_tables; spl_idx++) {
    ...
    if (type == OP_INSERT) {
      platform_throttled_error_log(
          ...
          base[spl_idx] / (total_ops[spl_idx] / 100), spl_idx);

      base[spl_idx] = __sync_fetch_and_add(&curr_op[spl_idx], op_granularity);
      ...
      if (PRG_step(rs) % 100 < lookup_positive_pct) {
        if (local_curr_op == op_granularity) {
          ...
          random_base =
              ((PRG_step(rs) % (local_curr_op / op_granularity - 1)) + 1) *
              op_granularity;
        }
      } else {
        if (local_curr_op >= total_ops[spl_idx] - op_granularity) {
          ...
          random_base =
              (PRG_step(rs) / op_granularity) * op_granularity + local_curr_op;
        } else {
          // negative lookup by select random base
          // [local_curr_op + op_granularity, total_ops - op_granularity]
          random_base =
              ((PRG_step(rs) %
                ((total_ops[spl_idx] - local_curr_op) / op_granularity - 1)) +
               1) *
                  op_granularity +
              local_curr_op;
        }
...
static void do_operation(test_splinter_thread_params *params,
                         const uint64 *base, uint64 num_ops, uint64 op_offset,
                         const uint8 *done, bool is_insert) {
  splinter_handle **spl_tables = params->spl;
  const test_config *test_cfg = params->test_cfg;
  uint64 op_granularity = params->op_granularity;
  uint64 thread_number = params->thread_number;
  ...
  for (uint64 op_idx = op_offset; op_idx != op_offset + num_ops; op_idx++) {
    ...
    for (uint8 spl_idx = 0; spl_idx < num_tables; spl_idx++) {
      ...
      if (is_insert) {
        test_key(key, test_cfg[spl_idx].key_type, op_num, thread_number,
                 test_cfg[spl_idx].semiseq_freq, splinter_key_size(spl),
                 test_cfg[spl_idx].period);
        test_insert_data((data_handle *)data, 1, (char *)&op_num, 8,
                         splinter_message_size(spl), type);
        ts = platform_get_timestamp();
        platform_status rc = splinter_insert(spl, key, data);
        platform_assert_status_ok(rc);
        ...
        if (async_lookup->max_async_inflight == 0) {
          ...
          ctxt = test_async_ctxt_get(spl, async_lookup, &vtarg);
          test_key(ctxt->key, test_cfg[spl_idx].key_type, op_num, thread_number,
                   test_cfg[spl_idx].semiseq_freq, splinter_key_size(spl),
                   test_cfg[spl_idx].period);
 ...
 *-----------------------------------------------------------------------------
 */
void test_splinter_insert_lookup_thread(void *arg) {
  test_splinter_thread_params *params = (test_splinter_thread_params *)arg;
  ...
  for (uint8 i = 0; i < NUM_OP_TYPES; i++) {
    bases[i] = TYPED_ARRAY_ZALLOC(platform_get_heap_id(), bases[i], num_tables);
  ...
  while (1) {
    for (uint8 op_type = 0; op_type != NUM_OP_TYPES; op_type++) {
      ...
      if (num_ops < granularities[op_type] ||
        ...
        if (advance_base(params, params->curr_op, bases[op_type], &insert_done,
                         &rs, op_type)) {
          goto out;
        }
        offsets[op_type] = 0;
...
test_splinter_create_tables(splinter_handle ***spl_handles,
                            splinter_config *cfg, allocator *al, cache *cc[],
                            task_system *ts, platform_heap_id hid,
                            uint8 num_tables, uint8 num_caches) {
...
static void test_splinter_destroy_tables(splinter_handle **spl_tables,
...
platform_status test_splinter_perf(splinter_config *cfg, test_config *test_cfg,
                                   allocator *al, cache *cc[],
                                   uint64 num_insert_threads,
                                   uint64 num_lookup_threads,
                                   uint64 num_range_threads,
                                   uint32 max_async_inflight, task_system *ts,
                                   platform_heap_id hid, uint8 num_tables,
                                   uint8 num_caches, uint64 insert_rate) {
  ...
  if (num_lookup_threads != 0) {
    ...
    for (uint64 i = 0; i < num_lookup_threads; i++) {
      ...
      if (params[i].lookup_stats[ASYNC_LU].latency_max >
        ...
        async_lookup_latency_max = params[i].lookup_stats[ASYNC_LU].latency_max;
      }
      if (!SUCCESS(params[i].rc)) {
        rc = params[i].rc;
        goto destroy_splinter;
...
platform_status
test_splinter_periodic(splinter_config *cfg, test_config *test_cfg,
                       allocator *al, cache *cc[], uint64 num_insert_threads,
                       uint64 num_lookup_threads, uint64 num_range_threads,
                       uint32 max_async_inflight, task_system *ts,
                       platform_heap_id hid, uint8 num_tables, uint8 num_caches,
                       uint64 insert_rate) {
  platform_log("splinter_test: splinter performance test started with %d\
                tables\n",
               num_tables);
...
platform_status test_splinter_parallel_perf(
    splinter_config *cfg, test_config *test_cfg, allocator *al, cache *cc[],
    uint64 seed, const uint64 num_threads, const uint64 num_inserts_per_thread,
    const uint64 num_lookups_per_thread, uint32 max_async_inflight,
    uint8 lookup_positive_pct, task_system *ts, platform_heap_id hid,
    uint8 num_tables, uint8 num_caches) {
  platform_log("splinter_test: splinter performance test started with %d\
                tables\n",
               num_tables);
  ...
  for (uint64 i = 0; i < num_threads; i++) {
    ...
    ret = task_thread_create(
        "insert/lookup thread", test_splinter_insert_lookup_thread, &params[i],
...
platform_status test_splinter_delete(splinter_config *cfg,
                                     test_config *test_cfg, allocator *al,
                                     cache *cc[], uint64 num_insert_threads,
                                     uint64 num_lookup_threads,
                                     uint32 max_async_inflight, task_system *ts,
                                     platform_heap_id hid, uint8 num_tables,
                                     uint8 num_caches, uint64 insert_rate) {
...
void test_splinter_shadow_insert(splinter_handle *spl, char *shadow, char *key,
                                 char *data, uint64 idx) {
...
int test_splinter_key_compare(const void *left, const void *right, void *spl) {
  return splinter_key_compare((splinter_handle *)spl, (const char *)left,
                              (const char *)right);
}

void *test_splinter_bsearch(
    register const void *key, void *base0, size_t nmemb, register size_t size,
    register int (*compar)(const void *, const void *, void *), void *ctxt) {
...
static platform_status
test_splinter_basic(allocator *al, cache *cc, splinter_config *cfg,
                    uint64 num_inserts, uint32 max_async_inflight,
                    task_system *ts, platform_heap_id hid) {
  ...
  for (insert_num = 0; insert_num < num_inserts; insert_num++) {
    ...
    if (max_async_inflight == 0) {
      ...
      verify_tuple(spl, insert_num, key, data, expected_data, data_size, TRUE,
                   found);
    } else {
      ...
      async_ctxt_process_one(spl, async_lookup, ctxt, NULL,
                             verify_tuple_callback, &vtarg_true);
    }
  }
  if (max_async_inflight != 0) {
  ...
  for (insert_num = num_inserts; insert_num < 2 * num_inserts; insert_num++) {
    ...
    if (max_async_inflight == 0) {
      ...
      ctxt->lookup_num = insert_num;
      async_ctxt_process_one(spl, async_lookup, ctxt, NULL,
                             verify_tuple_callback, &vtarg_false);
    }
  }
  if (max_async_inflight != 0) {
  ...
  platform_sort_slow(shadow, num_inserts, tuple_size, test_splinter_key_compare,
                     spl, temp);
  ...
  for (uint64 range_num = 0; range_num != num_ranges; range_num++) {
    ...
    char start_key[MAX_KEY_SIZE];
    test_key(start_key, TEST_RANDOM, num_inserts + range_num, 0, 0, key_size,
             0);
    uint64 range_tuples = test_range(range_num, 1, 100);
    uint64 returned_tuples = 0;
    rc = splinter_range(spl, start_key, range_tuples, &returned_tuples,
                        range_output);
    platform_assert_status_ok(rc);
    char *shadow_start =
        test_splinter_bsearch(start_key, shadow, num_inserts, tuple_size,
                              test_splinter_key_compare, spl);
    ...
    if (returned_tuples != expected_returned_tuples ||
        memcmp(shadow_start, range_output, returned_tuples * tuple_size) != 0) {
...
static int splinter_test_parse_perf_args(char ***argv, int *argc,
...
int splinter_test(int argc, char *argv[]) {
  io_config io_cfg;
  ...
  if (argc > 1 && strncmp(argv[1], "--perf", sizeof("--perf")) == 0) {
    ...
    num_range_lookup_threads = 10;
             ...
             strncmp(argv[1], "--delete", sizeof("--delete")) == 0) {
    test = delete;
    config_argc = argc - 2;
    config_argv = argv + 2;
    num_insert_threads = 14;
    ...
    config_argc = argc - 2;
    config_argv = argv + 2;
  } else if (argc > 1 && strncmp(argv[1], "--semiseq-perf",
                                 sizeof("--semiseq-perf")) == 0) {
    test = semiseq_perf;
    config_argc = argc - 2;
    config_argv = argv + 2;
  } else if (argc > 1 && strncmp(argv[1], "--parallel-perf",
                                 sizeof("--parallel-perf")) == 0) {
    test = parallel_perf;
    config_argc = argc - 2;
    config_argv = argv + 2;
    num_pthreads = 10;
    lookup_positive_pct = 50;
  } else if (argc > 1 && strncmp(argv[1], "--functionality",
                                 sizeof("--functionality")) == 0) {
    if (argc < 4) {
      usage(argv[0]);
      return -1;
    }
  ...
  if (splinter_test_parse_perf_args(
          &config_argv, &config_argc, &max_async_inflight, &num_insert_threads,
          &num_lookup_threads, &num_range_lookup_threads, &num_pthreads,
  ...
  if (config_argc > 0 &&
      strncmp(config_argv[0], "--insert-rate", sizeof("--insert-rate")) == 0) {
    if (config_argc < 2) {
      usage(argv[0]);
      return -1;
    }
  ...
  platform_heap_id hid;
  rc = platform_heap_create(platform_get_module_id(), heap_capacity, &hh, &hid);
  platform_assert_status_ok(rc);
  ...
  clockcache_config *cache_cfg = TYPED_ARRAY_MALLOC(hid, cache_cfg, num_tables);
  ...
  if (io_cfg.kernel_queue_size < total_threads * max_async_inflight) {
    io_cfg.kernel_queue_size = ROUNDUP(total_threads * max_async_inflight, 32);
  ...
  rc = test_init_splinter(hid, io, &ts, splinter_cfg->use_stats, use_bg_threads,
                          num_bg_threads);
  if (!SUCCESS(rc)) {
  ...
  case perf:
    rc = test_splinter_perf(splinter_cfg, test_cfg, alp, caches,
                            ...
                            num_range_lookup_threads, max_async_inflight, ts,
                            hid, num_tables, num_caches, insert_rate);
    platform_assert(SUCCESS(rc));
    break;
  case delete:
  ...
  case parallel_perf:
    ...
    rc = test_splinter_parallel_perf(splinter_cfg, test_cfg, alp, caches, seed,
                                     num_pthreads, 3, 7, max_async_inflight,
                                     lookup_positive_pct, ts, hid, num_tables,
                                     num_caches);
    platform_assert_status_ok(rc);
    break;
  case periodic:
    rc = test_splinter_periodic(splinter_cfg, test_cfg, alp, caches,
                                ...
                                ts, hid, num_tables, num_caches, insert_rate);
    platform_assert(SUCCESS(rc));
    break;
  case functionality:
    for (uint8 i = 0; i < num_tables; i++) {
    ...
    rc = test_functionality(alp, (io_handle *)io, caches, splinter_cfg, seed,
                            test_ops, correctness_check_frequency, ts, hh, hid,
