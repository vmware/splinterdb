...
/*
 * Test case to verify conversion of size value to a string with unit-specifiers.
 * Do quick cross-check of the lower-level conversion macros involved.
 */
CTEST2(misc, test_size_to_str)
{
   ...
   for (int cctr = 0; cctr < ARRAY_SIZE(size_to_str_cases); cctr++) {
      ...
      test_streqn(expstr, size_str, cctr);
   }
}
