...
CTEST2(writable_buffer, test_writable_buffer_append)
{
   ...
   ASSERT_TRUE(STRING_EQUALS_LITERAL(writable_buffer_data(wb), "Hello World!"));

   input_str = "Hello Another World!";
   writable_buffer_append(wb, strlen(input_str), (const void *)input_str);
   ASSERT_TRUE(STRING_EQUALS_LITERAL(writable_buffer_data(wb),
