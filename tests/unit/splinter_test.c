...
/*
 * -----------------------------------------------------------------------------
 * Test case to exercise verbose print logging instrumentation in trunk.c
...
CTEST2(splinter, test_splinter_verbose_print_diags)
{
   ...
   trunk_enable_verbose_logging(spl, Platform_default_log_handle);

   // Insert slightly over a 1M rows
   data->test_exec_cfg.num_inserts = (Kilo * Kilo);
   ...
   trunk_disable_verbose_logging(spl);
