...
CTEST2(splinter, test_lookups)
{
   ...
   num_inserts = splinter_do_inserts(data, spl, FALSE, &shadow, &tuple_size);
   ASSERT_NOT_EQUAL(0,
                    num_inserts,
                    "Expected to have inserted non-zero rows, num_inserts=%lu.",
                    num_inserts);
...
CTEST2(splinter, test_splinter_print_diags)
{
   ...
   int    tuple_size = 0;
   uint64 num_inserts =
      splinter_do_inserts(data, spl, FALSE, NULL, &tuple_size);
   ASSERT_NOT_EQUAL(0,
                    num_inserts,
                    "Expected to have inserted non-zero rows, num_inserts=%lu"
                    ", tuple_size=%d.",
   ...
   platform_log("**** Splinter Diagnostics ****\n"
                "Generated by %s:%d:%s ****\n",
                __FILE__,
                __LINE__,
                __FUNCTION__);

   trunk_print_super_block(spl);

   trunk_print_space_use(spl);

   platform_log("\n** trunk_sprint() **\n");
   trunk_sprint(Platform_stdout_fh, spl);

   platform_log("\n** Allocator stats **\n");
   rc_allocator_sprint_stats(Platform_stdout_fh, (rc_allocator *)alp);
   rc_allocator_sprint_debug(Platform_stdout_fh, (rc_allocator *)alp);
