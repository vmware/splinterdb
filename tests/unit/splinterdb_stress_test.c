...
CTEST2(splinterdb_stress, test_iterator_over_many_kvs)
{
   ...
   ASSERT_EQUAL(0, splinterdb_iterator_init(data->kvsb, &it, start_key, greater_than_or_equal));
...
naive_range_delete(splinterdb *kvsb, slice start_key, uint32 count)
{
   ...
   int rc = splinterdb_iterator_init(kvsb, &it, start_key, greater_than_or_equal);
   ASSERT_EQUAL(0, rc);
