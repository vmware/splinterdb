...
leaf_hdr_tests(btree_config *cfg, btree_scratch *scratch)
{
   char       leaf_buffer[cfg->cache_cfg->page_size];
...
index_hdr_tests(btree_config *cfg, btree_scratch *scratch)
{
   char       index_buffer[cfg->cache_cfg->page_size];
...
{
   char leaf_buffer[cfg->cache_cfg->page_size];
   char msg_buffer[cfg->cache_cfg->page_size];
   ...
   int   msgsize = cfg->cache_cfg->page_size / (nkvs + 1);
