...
static void verify_tuple(splinter_handle *spl, char *keybuf, data_handle *msg,
                         int8 refcount, bool found, platform_status *result) {
  if (refcount && !found) {
    ...
    *result = STATUS_NOT_FOUND;
    ...
    platform_error_log(
        ...
        key, msg->message_type, msg->ref_count);
...
static void verify_tuple_callback(splinter_handle *spl, test_async_ctxt *ctxt,
  ...
  verify_tuple(spl, ctxt->key, (data_handle *)ctxt->data, ctxt->refcount, found,
               result);
}
...
platform_status verify_against_shadow(splinter_handle *spl, char *keybuf,
                                      data_handle *msg,
...
platform_status verify_range_against_shadow(
    splinter_handle *spl, test_splinter_shadow_array *sharr, char *start_key,
    char *end_key, platform_heap_id hid, uint64 start_index, uint64 end_index) {
  ...
  status = splinter_range_iterator_init(spl, range_itor, start_key, end_key,
  ...
  for (i = start_index; i < end_index; i++) {
    ...
    iterator_get_curr((iterator *)range_itor, (char **)&splinter_keybuf,
                      (char **)&splinter_data, &type);
    ...
    if (splinter_key == shadow_key) {
      ...
      platform_error_log(
          ...
          shadow_key, shadow_refcount, splinter_key,
          splinter_data->message_type, splinter_data->ref_count);
      platform_assert(0);
  ...
  while (SUCCESS(iterator_at_end((iterator *)range_itor, &at_end)) && !at_end) {
    ...
    platform_log("Range iterator EXTRA KEY: %08lx \n"
                 "Tree Msg Type: 0x%02x, Tree Refcount %3d\n",
                 splinter_key, splinter_data->message_type,
...
static void *choose_key(data_config *cfg,                  // IN
                        ...
                        void *keybuf)                      // OUT
{
  ...
  case VERIFY_RANGE_ENDPOINT_RAND: {
    ...
    uint64 pos;
    pos =
        num_keys / 5 + (PRG_step(prg) % ((num_keys < 5) ? (num_keys + 1) / 2
                                                        : (3 * num_keys / 5)));
...
platform_status
verify_range_against_shadow_all_types(splinter_handle *spl, PRG *prg,
                                      test_splinter_shadow_array *sharr,
                                      platform_heap_id hid, bool do_it) {
  int begin_type;
  int end_type;
  platform_status rc, result = STATUS_OK;
  ...
  for (begin_type = VERIFY_RANGE_ENDPOINT_NULL;
       begin_type <= VERIFY_RANGE_ENDPOINT_RAND; begin_type++)
    for (end_type = VERIFY_RANGE_ENDPOINT_NULL;
         end_type <= VERIFY_RANGE_ENDPOINT_RAND; end_type++) {
      start_key = choose_key(spl->cfg.data_cfg, sharr, prg, begin_type, 1, NULL,
                             0, &start_index, startkey_buf);
      end_key = choose_key(spl->cfg.data_cfg, sharr, prg, end_type, 0,
                           start_key, start_index, &end_index, endkey_buf);
      if (do_it) {
        rc = verify_range_against_shadow(spl, sharr, start_key, end_key, hid,
  ...
  for (end_type = VERIFY_RANGE_ENDPOINT_EQUAL;
       end_type <= VERIFY_RANGE_ENDPOINT_LESS; end_type++) {
    start_key = choose_key(spl->cfg.data_cfg, sharr, prg, begin_type, 1, NULL,
                           0, &start_index, startkey_buf);
    end_key = choose_key(spl->cfg.data_cfg, sharr, prg, end_type, 0, start_key,
                         start_index, &end_index, endkey_buf);
    if (do_it) {
      rc = verify_range_against_shadow(spl, sharr, start_key, end_key, hid,
                                       start_index, end_index);
      if (!SUCCESS(rc)) {
...
static platform_status validate_tree_against_shadow(
    splinter_handle *spl, PRG *prg, char *keybuf, data_handle *msg,
    test_splinter_shadow_tree *shadow, platform_heap_handle hh,
    platform_heap_id hid, bool do_it, test_async_lookup *async_lookup) {
...
insert_random_messages(splinter_handle *spl, test_splinter_shadow_tree *shadow,
                       PRG *prg, char *keybuf, data_handle *msg,
                       int num_messages, message_type op, uint64 minkey,
                       uint64 maxkey, int64 mindelta, int64 maxdelta) {
  uint64 key_size = spl->cfg.data_cfg->key_size;
  uint64 data_size = spl->cfg.data_cfg->message_size;
  ...
  for (i = 0; i < num_messages; i++) {
    ...
    key = minkey + (((key - minkey) + mindelta +
                     (PRG_step(prg) % (maxdelta - mindelta + 1))) %
                    (maxkey - minkey + 1));
...
int cmp_ptrs(const void *a, const void *b) { return a - b; }
...
platform_status test_functionality(
    allocator *al, io_handle *io, cache *cc[], splinter_config *cfg,
    uint64 seed, uint64 num_inserts, uint64 correctness_check_frequency,
    task_system *state, platform_heap_handle hh, platform_heap_id hid,
    uint8 num_tables, uint8 num_caches, uint32 max_async_inflight) {
  ...
  for (uint8 idx = 0; idx < num_tables; idx++) {
    ...
    status = validate_tree_against_shadow(spl, &prg, keybuf, msgbuf, shadow, hh,
                                          hid, TRUE, async_lookup);
    if (!SUCCESS(status)) {
  ...
  while (total_inserts < num_inserts) {
    ...
    platform_log("Round i=%8lu, op=%2d, nummsgs = %8d, minkey=%8lu, "
                 ...
                 i, op, num_messages, minkey, maxkey, mindelta, maxdelta);
    ...
    for (uint8 idx = 0; idx < num_tables; idx++) {
      ...
      status = insert_random_messages(spl, shadow, &prg, keybuf, msgbuf,
                                      num_messages, op, minkey, maxkey,
                                      mindelta, maxdelta);
      if (!SUCCESS(status)) {
      ...
      status = validate_tree_against_shadow(
          spl, &prg, keybuf, msgbuf, shadow, hh, hid,
